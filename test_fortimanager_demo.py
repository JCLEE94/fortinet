...
def test_fortimanager_demo():
    ...
    - API Key: 8wy6xtig45xkn8oxukmiegf5yn18rn4c
    """
    ...
    try:
        ...
        try:
            ...
            if devices:
                ...
                for device in devices:
                    logger.info(f"  - {device.get('name', 'unknown')} ({device.get('ip', 'no-ip')}) - Status: {device.get('conn_status', 'unknown')}")
            else:
        ...
        if test_results['discovered_data'].get('managed_devices'):
            ...
            if device_name:
                ...
                try:
                    test_results['api_endpoints_tested'].append(f'/pm/config/device/{device_name}/vdom/root/firewall/policy')
                ...
                try:
                    test_results['api_endpoints_tested'].append(f'/pm/config/device/{device_name}/global/system/interface')
        ...
        test_results['test_duration'] = str(datetime.fromisoformat(test_results['test_end_time']) - 
