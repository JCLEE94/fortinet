            ...
            src_route = (
                         self.routing_table.get(f"{src_ip}/32")
                             or self.routing_table.get(
                                                       CONFIG.network.INTERNAL_NETWORKS[
                                                           0] if CONFIG.network.INTERNAL_NETWORKS else "192.168.0.0/16"
                                              )
            ...
            for network, route in self.routing_table.items():
                try:
                    if ipaddress.ip_address(
                                            dst_ip) in ipaddress.ip_network(network):
                        dst_route = route
                        break
                except Exception:
                    continue
                ...
                hop = {
                    "hop_number": i + 1,
                        ...
                        "src_ip": route["from"],
                            "dst_ip": route["to"],
                                # 첫 번째 홉에서만 정책 적용
                                "policy_id": (policy_id if i == 0 else None),
                ...
                recommendations.append(
                                       ...
                                       "type": "error",
                                           "message": f"트래픽이 정책 '{policy['name']}'에 의해 차단됩니다.",
                                               "action": (f"필요한 경우 {self.get_zone_for_ip(src_ip)}에서 {self.get_zone_for_ip(dst_ip)}로의 "
                                "접근을 허용하는 정책을 추가하세요."),
                                    }
                                  )
