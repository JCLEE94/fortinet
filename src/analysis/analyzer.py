...
logger = setup_logger("analyzer")
    ...
    def load_data(self, firewall_id="default"):
        ...
        try:
            if self.fortigate_client:
                ...
                # 정책 패키지 정보 로드
                    policy_packages = self.fortimanager_client.get_policy_packages(
                                                                                   adom)
                ...
                policy_package = policy_packages[0]["name"] if policy_packages and len(
                                                                                       policy_packages) > 0 else None

                    if policy_package:
                                                self._policies[firewall_id] = self.fortimanager_client.get_firewall_policies(
                                                                                                                             policy_package,
                                                                                                                                 adom
                        )
                ...
                # 라우팅 테이블 로드
                                        self._routing_tables[firewall_id] = self.fortimanager_client.get_device_routing_table(
                                                                                                                              firewall_id,
                                                                                                                                  adom
                    )
    ...
    def _determine_firewall_path(self, src_ip, dst_ip):
        ...
        try:
            ...
            # IP 주소 객체 생성 (향후 확장 시 사용 예정)
            _src_ip_obj = ipaddress.ip_address(src_ip)
                _dst_ip_obj = ipaddress.ip_address(dst_ip)
            ...
            # 단일 방화벽 환경의 경우
                ...
                egress_iface = (
                                egress_route.get(
                                                 "interface") if egress_route else self._find_interface_for_ip(dst_ip,
                                                                                                               firewall_id
                                    )
                                )
            ...
            while next_firewall_idx < len(relevant_firewalls) and hop_count < max_hops:
                ...
                is_last = next_firewall_idx == len(relevant_firewalls) - 1
                    ...
                    self.logger.info(f"방화벽 {firewall_id} 분석 중 (현재 IP: {_current_ip}, 목적지: {dst_ip})")

                # 현재 소스 IP에 대한 라우팅 정보
                    ingress_route = self._find_route_for_ip(_current_ip, firewall_id)
                ...
                # 인터페이스 정보 결정
                    ingress_iface = (
                                     ingress_route.get("interface")
                                     if ingress_route
                    else self._find_interface_for_ip(_current_ip, firewall_id)
                        )
                ...
                if is_last or (dst_route and not dst_route.get("gateway")):
                    ...
                    egress_iface = (
                                    dst_route.get(
                                                  "interface") if dst_route else self._find_interface_for_ip(dst_ip,
                                                                                                             firewall_id
                                        )
                                    )
                    next_hop = dst_ip
                    ...
                    if src_any and dst_any and svc_any:
                        recommendations.append(
                                            ...
                                            "type": "security_improvement",
                                                "issue": "ALL 설정 사용",
                                                    "description": (f"방화벽 '{firewall_id}'의 정책 {p_data['policy_id']}에서 "
                                ...
                                recommendations.append(
                                                       {
                                                           "firewall_id": firewall_id,
                                                               "policy_ids": [p.get("policy_id") for p in policies],
                                                                     ...
                                                                     "issue": "동일 구간 다중 정책",
                                                                         "description": (f"방화벽 '{firewall_id}'에서 {zones[0]} -> {zones[1]} 구간에 "
            ...
            for firewall_id, data in analyzed_firewalls.items():
                ...
                if len(data["zones"]) > 2:
                    recommendations.append(
                                        ...
                                        "issue": "다중 구간 관리",
                                            "description": (
                                                            f"방화벽 '{firewall_id}'가 {', '.join(data['zones'])} 등 여러 구간을 동시에 관리하고 있습니다.",
                                                                                              )
                        ...
                        if not logtraffic or logtraffic == "disable":
                            recommendations.append(
                                            ...
                                            "type": "logging_improvement",
                                                "issue": "로그 비활성화",
                                                    "description": (
                                                                    f"방화벽 '{firewall_id}'의 정책 {p_data['policy_id']}에서 트래픽 로깅이 비활성화되어 있습니다.",
                                                                        )
                                                       ...
                                                       "end_time": session_data.get("end_time", ""),
                                                                                    "status": session_data.get(
                                                                                                               "status",
                                                                                                                                                                        ...
                                                                                                                                                                        "src_ip": session_data.get(
                                                                                                                                                                                                   "source_ip",
                                                                                                                                                                                                       ""
                                                                                    )
                                                                                                                                                                                "dst_ip": session_data.get(
                                                                                                                                                                                                           "destination_ip",
                                                                                                                                                                                                               ""
                                                                                        )
                                                                                                                                                                            "port": session_data.get(
                                                                                                                                                                                                     "port",
                                                                                                                                                                                                         ""
                                                                                      )
                                                                                                                                                                        "protocol": session_data.get(
                                                                                                                                                                                                     "protocol",
                                                                                                                                                                                                         ""
                                                                                    )
                                                                                                                                                                                "allowed": session_data.get(
                                                                                                                                                                                                            "allowed",
                                                                                                                                                                                                                False
                                                                                        )
                                                                                                                                                                          "progress": session_data.get(
                                                                                                                                                                                                       "progress",
                                                                                                                                                                                                           0
                                                                                     )
                                                                                          }
