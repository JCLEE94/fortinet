        ...
        try:
            ...
            for i, hop in enumerate(firewall_path):
                ...
                network_hop = {
                    ...
                    'destination_ip': dst_ip if is_last else (
                        firewall_path[i+1].get('next_hop', 'unknown') if not is_last else 'unknown'
                    ),
    ...
    def _determine_firewall_path(self, src_ip, dst_ip):
        ...
        try:
            ...
            if len(relevant_firewalls) == 1:
                ...
                ingress_iface = ingress_route.get('interface') if ingress_route else self._find_interface_for_ip(src_ip, firewall_id)
                egress_iface = egress_route.get('interface') if egress_route else self._find_interface_for_ip(dst_ip, firewall_id)
            ...
            while next_firewall_idx < len(relevant_firewalls) and hop_count < max_hops:
                ...
                # 인터페이스 정보 결정
                ingress_iface = ingress_route.get('interface') if ingress_route else self._find_interface_for_ip(current_ip, firewall_id)

                # 목적지까지의 경로가 직접적으로 연결되어 있는지 확인
                if is_last or (dst_route and not dst_route.get('gateway')):
                    # 마지막 방화벽이거나 직접 연결된 경우
                    egress_iface = dst_route.get('interface') if dst_route else self._find_interface_for_ip(dst_ip, firewall_id)
                    next_hop = dst_ip
                    ...
                    if dst_route and dst_route.get('gateway'):
                        ...
                        for idx, fw_id in enumerate(relevant_firewalls[next_firewall_idx+1:], next_firewall_idx+1):
                        ...
                        next_firewall_idx += 1
                        next_hop = self._estimate_next_hop(firewall_id,
                                                          relevant_firewalls[next_firewall_idx] if next_firewall_idx < len(relevant_firewalls) else None,
                                                          dst_ip)
                        egress_iface = self._find_egress_interface(next_hop, firewall_id)

                # 인터페이스 구간 정보 결정
                ingress_zone = self._get_interface_zone(ingress_iface, firewall_id) or (src_zone if is_first else current_zone)
                egress_zone = self._get_interface_zone(egress_iface, firewall_id) or (dst_zone if is_last else "transit")

                # 방화벽 홉 유형 결정
                hop_type = "source" if is_first else ("destination" if is_last or next_firewall_idx >= len(relevant_firewalls) else "transit")
    ...
    def _identify_relevant_firewalls_from_routing(self, src_ip, dst_ip):
        ...
        try:
            ...
            if not has_routing_data:
                ...
                return self._identify_relevant_firewalls(src_ip, dst_ip,
