...
logger = setup_logger("policy_analyzer")
    ...
    def analyze_traffic(self, src_ip, dst_ip, dst_port,
                        protocol="tcp", firewall_id="default"):
        """
    ...
    def _policy_matches_traffic(
        ...
        try:
            ...
            # 목적지 주소 확인
            if not self._check_destination_addresses(
                                                     policy, dst_ip, firewall_id):
                return False

            # 서비스 확인
            if not self._check_services(
                                        policy, dst_port, protocol, firewall_id):
                return False
