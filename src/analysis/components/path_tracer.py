...
class PathTracer:
    ...
    def _is_potential_loop(self, route1, route2):
        ...
        if gateway1 and gateway2:
            try:
                if (ipaddress.ip_address(gateway1) in ipaddress.ip_network(dest2, strict=False) and
                    ipaddress.ip_address(gateway2) in ipaddress.ip_network(dest1, strict=False)):
                    return True
            except ValueError:
                pass
