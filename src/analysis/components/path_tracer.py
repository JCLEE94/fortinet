                                        ...
                                        "type": "ingress",
                                            "interface": src_interface.get("name", "unknown"),
                                                           "network": src_interface.get("network", "unknown"),
                                                                                        "description": f"패킷이 {src_interface.get(
                                                                                                                                'name',
                                                                                                                                    'unknown')} 인터페이스로 유입"
                                                                   )
                                                               }
        ...
        if dst_route:
            ...
            route_type= dst_route.get("type", "unknown")
                ...
                if route_type == "connected":
                    _description= f"목적지가 직접 연결된 네트워크 {destination}에 있음"
            elif gateway:
                _description= f"게이트웨이 {gateway}를 통해 {interface} 인터페이스로 라우팅"
            else:
                _description= f"{interface} 인터페이스로 직접 라우팅"
