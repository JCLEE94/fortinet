...
(function() {
    ...
    // 테스트 모드 감지
    const isTestMode = window.APP_MODE === 'test' || 
                      document.body.dataset.appMode === 'test' ||
                      window.location.search.includes('test=true');
    
    // 오프라인 모드 감지
    const isOfflineMode = window.OFFLINE_MODE === true ||
    ...
    function showModeDetails() {
        const details = {
            ...
            'Build Time': document.querySelector('meta[name="build-time"]')?.content || 'unknown'
        };
    ...
    function logModeInfo() {
        ...
        console.log(`%c  APP_MODE: ${window.APP_MODE || 'production'}`, styles.info);
        console.log(`%c  OFFLINE_MODE: ${isOfflineMode}`, isOfflineMode ? styles.warning : styles.info);
        console.log(`%c  Test Data: ${isTestMode ? 'Active' : 'Inactive'}`, isTestMode ? styles.warning : styles.info);
        
        if (isTestMode) {
            console.log('%c⚠️ Application is running in TEST MODE with dummy data', styles.warning);
        }
        
        if (isOfflineMode) {
            console.log('%c🔒 Application is running in OFFLINE MODE - external calls blocked', styles.warning);
        }
        
        console.log('%cFor production use, ensure APP_MODE=production and OFFLINE_MODE=false', styles.info);
    }
    ...
    function setupPerformanceMonitoring() {
        ...
        window.addEventListener('load', function() {
            setTimeout(() => {
                ...
                console.log(`%c⏱️ Page Load Time: ${loadTime}ms`, 'color: #9C27B0; font-size: 12px;');
        ...
        window.fetch = function(...args) {
            ...
            return originalFetch.apply(this, args)
                .then(response => {
                    ...
                    console.log(`%c🌐 API Call: ${url} (${duration}ms)`, 
                ...
                .catch(error => {
                    ...
                    console.error(`%c❌ API Error: ${url} (${duration}ms)`, 
                                 'color: #F44336; font-size: 11px;', error);
