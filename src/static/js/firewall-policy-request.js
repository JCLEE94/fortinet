...
function updateRiskAssessment() {
    ...
    // 필수 필드가 모두 입력되었는지 확인
    if (!formData.source_ip || !formData.destination_ip || !formData.port || !formData.protocol || !formData.action) {
        hideRiskIndicator();
        return;
    }
...
function calculateRiskLevel(data) {
    ...
    if (data.action === 'allow') {
        // 관리 포트 체크
        const managementPorts = ['22', '3389', '23', '21', '25', '110', '143', '993', '995'];
        ...
        // 소스 IP 범위 체크
        if (data.source_ip.includes('any') || data.source_ip.includes('0.0.0.0') || data.source_ip.includes('/0')) {
            riskLevel = 'high';
            riskReasons.push('모든 IP 대역 허용');
        }
        
        // 포트 범위 체크
        if (data.port.includes('-') || data.port.includes('any') || data.port.includes('all')) {
            if (riskLevel === 'low') {
                riskLevel = 'medium';
            }
...
async function submitPolicyRequest(event) {
    ...
    try {
        ...
        if (!analysisResult.success) {
            showErrorMessage(analysisResult.message || '정책 분석 중 오류가 발생했습니다.');
            return;
        }
        ...
        if (ticketResult.success) {
            showSuccessMessage(`방화벽 정책 요청이 성공적으로 생성되었습니다. 티켓 ID: ${ticketResult.ticket_info.ticket_id}`);
            ...
            showErrorMessage(ticketResult.message || 'ITSM 티켓 생성 중 오류가 발생했습니다.');
        }
...
function displayAnalysisResults(apiResult) {
    ...
    if (recommendationsList && securityConsiderations) {
        recommendationsList.innerHTML = securityConsiderations
            .map(consideration => `<div class="recommendation-item">${consideration}</div>`)
            .join('');
    }
...
function showLoading(show) {
    ...
    if (submitBtn) {
        if (show) {
            ...
            submitBtn.innerHTML = '<i class="fas fa-paper-plane"></i> 방화벽 정책 요청 제출';
        }
    }
}
...
function validateForm() {
    const requiredFields = ['title', 'requester', 'source_ip', 'destination_ip', 'port', 'protocol', 'action'];
    let isValid = true;
...
document.addEventListener('input', function(event) {
    ...
    if (field.name === 'port') {
        if (field.value && !isValidPort(field.value) && !field.value.toLowerCase().includes('any')) {
            field.classList.add('invalid');
