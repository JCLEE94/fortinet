...
const NextradeUtils = {
    ...
    async apiRequest(endpoint, options = {}) {
        ...
        for (let attempt = 1; attempt <= NextradeConfig.api.retryAttempts; attempt++) {
            try {
...
const NextradeAPI = {
    ...
    async getRequestDetail(requestId) {
        return await NextradeUtils.apiRequest(`/api/itsm/request-detail/${requestId}`);
    },
    ...
    async createFirewallTicket(data) {
        return await NextradeUtils.apiRequest('/api/firewall-policy/create-ticket', {
            method: 'POST',
            body: JSON.stringify(data)
        });
    },
...
const NextradeUI = {
    ...
    enhanceTable(table) {
        ...
        if (searchInput) {
            searchInput.addEventListener('input', 
                NextradeUtils.debounce(() => this.filterTable(table, searchInput.value), 300)
            );
        }
    ...
    addTextOverflowHandling(table) {
        const overflowCells = table.querySelectorAll('.text-overflow, .table-col-xs, .table-col-sm, .table-col-md, .table-col-lg, .table-col-xl');
    ...
    fixLayoutIssues() {
        ...
        if (sidebar && content) {
            ...
            resizeSidebar();
            window.addEventListener('resize', NextradeUtils.debounce(resizeSidebar, 100));
        }
