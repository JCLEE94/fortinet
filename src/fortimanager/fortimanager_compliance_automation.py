...
class ComplianceCheckResult:
                      ...
                      ComplianceRule(
                                     rule_id="SEC-001",
                                         name="No Any-Any Policies",
                                             description = (
                                                 "Ensure no firewall policies allow any source to any destination",
                                             )
                                       category="security",
        ...
        self.add_rule(
                      ComplianceRule(
                                    ...
                                    check_function="check_ssl_inspection",
                                        ...
                                        frameworks=["PCI-DSS"],
                                        parameters={
                                        "required_profiles": ["deep-inspection"]},
                                            )
                            )
                                              ...
                                              category=rule_config["category"],
                                                  severity=ComplianceSeverity(
                                                            rule_config["severity"]),
                                                                ...
                                                                remediation_function=rule_config.get(
                                                                                                     "remediation_function"),
                                                                                                         parameters=rule_config.get(
                                                                                                                        "parameters",
                                                                                                                                                                                                                         {}
                                                                                             )
                                                                                                                                                                                frameworks=rule_config.get(
                                                                                                                                                                                                           "frameworks",
                                                                                                                                                                                                               []
                                                                                        )
                                                                                                                                                                                        auto_remediate=rule_config.get(
                                                                                                                                                                                                                       "auto_remediate",
                                                                                                                                                                                                                           False
                                                                                            )
                                                                                      )
                                             ...
                                             rule_id=rule.rule_id,
                                                 device=device,
                                                     status=ComplianceStatus.NON_COMPLIANT,
                                                         severity=rule.severity,
                                                             message = (
                                               f"{len(web_policies_without_ssl)} web policies without proper SSL inspection",
                                                      )
    ...
    async def _run_single_check(self, device: str, rule: ComplianceRule, adom: str) -> ComplianceCheckResult:
        ...
        # Run the check
        try:
            _result = await check_func(device, rule, adom)
                return _result
        except Exception as e:
            return ComplianceCheckResult(
                                         rule_id=rule.rule_id,
    ...
    async def _apply_remediation(
        ...
        # Apply remediation
        _result = await remediation_func(device, issue, adom)
