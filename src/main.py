...
# 공통 임포트 사용
from src.utils.common_imports import Optional, datetime, get_env_bool, json, os, setup_module_logger, sys
...
def parse_args():
    ...
    parser.add_argument(
        "--protocol", help="프로토콜 (tcp, udp, icmp)", choices=["tcp", "udp", "icmp"], default="tcp", required=False
    )
    ...
    parser.add_argument(
        "--log-level", help="로그 레벨", choices=["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"], default="INFO"
    )
...
def analyze_packet_path(src_ip, dst_ip, port, protocol, api_client, manager=False):
    ...
    try:
        analyzer = FirewallRuleAnalyzer(
            fortigate_client=None if manager else api_client, fortimanager_client=api_client if manager else None
        )
...
def main():
    ...
    if args.manager:
        ...
        api_client = FortiManagerAPIClient(
            host=args.host, api_token=args.token, username=args.username, password=args.password
        )
    else:
        # FortiGate API 클라이언트
