...
logger = setup_module_logger("main")
   ...
   def determine_target_environment() -> str:
    ...
    # Development environment indicators
        if node_env == "development" or app_mode == "test" or os.getenv(
...
def get_env_port(environment: str) -> int:
    ...
    if environment == "dev":
        return int(os.getenv("DEV_PORT", os.getenv(
                   "PORT", str(APP_CONFIG["web_port"]))))
           else:
                return int(os.getenv("PRD_PORT", os.getenv(
                           "PORT", str(APP_CONFIG["web_port"]))))

            def is_docker_environment() -> bool:
                """Check if running in Docker container"""
    return os.path.exists(
                          "/.dockerenv") or os.getenv("DOCKER_CONTAINER", "false").lower() == "true"
    ...
    environment = determine_target_environment()
       is_docker = is_docker_environment()
        ...
        logger.info(
                    f"Environment validation: {environment.upper()}, Docker: {is_docker}")
...
def parse_args():
    ...
    parser.add_argument("--output", help="출력 파일 경로", required=False)
       parser.add_argument("--web", help="웹 인터페이스 시작", action="store_true")
        parser.add_argument(
                            "--host",
                                help="FortiManager 또는 FortiGate 호스트",
     required=False)
    ...
    # 웹 인터페이스 시작
        ...
        build_time = os.getenv("BUILD_TIME", "Development")
           logger.info(f"Starting FortiGate Nextrade in {environment.upper()} environment on port {port}")
            logger.info(
                        f"Build: {build_time}, Docker: {is_docker_environment()}")
            logger.info(f"Project: {os.getenv('PROJECT_NAME', 'fortinet')}")

        # Check if Socket.IO is enabled
            socketio_enabled = not os.getenv(
                                             "DISABLE_SOCKETIO", "false").lower() == "true"
                ...
                logger.info("Socket.IO enabled - using socketio.run()")
                   socketio.run(app, host=host, port=port, debug=debug, allow_unsafe_werkzeug=True)
                    ...
                    app.run(host=host, port=port, debug=debug)
                        ...
                        logger.info(
                                    "Socket.IO disabled - using standard Flask")
                app.run(host=host, port=port, debug=debug)
                return
