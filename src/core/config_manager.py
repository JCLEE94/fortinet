...
class AppConfig:
    ...
    # FortiGate settings
    fortigate_verify_ssl: bool = os.getenv(
                                           "FORTIGATE_VERIFY_SSL", "false").lower() == "true"
        fortigate_timeout: int = TIMEOUTS["API_REQUEST"]

    # FortiManager settings
        fortimanager_verify_ssl: bool = os.getenv(
                                                  "FORTIMANAGER_VERIFY_SSL", "false").lower() == "true"
    ...
    # Monitoring settings
        monitoring_enabled: bool = os.getenv(
                                             "MONITORING_ENABLED", "true").lower() == "true"
    ...
    def __init__(self, base_dir: Optional[str] = None):
        ...
        for config_path in config_paths:
            ...
            if full_path.exists():
                format_type = ConfigFormat.YAML if config_path.endswith(
                                                                        ".yaml") else ConfigFormat.JSON
                    self.add_source(
                                    ConfigSource(
                                                 path=str(full_path),
                                                      format=format_type,
    ...
    def add_source(self, source: ConfigSource):
        ...
        # Create app config object
            ...
            def _load_source(
                             self, source: ConfigSource) -> Optional[Dict[str, Any]]:
                """
        ...
        if source.format == ConfigFormat.ENV:
            ...
            return self._load_yaml_config(source.path)
                    ...
                    raise ValueError(
                                     f"Unsupported config format: {source.format}")
    ...
    def _merge_configs(self, base: Dict[str, Any], override: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        _result = base.copy()
            self._merge_config(_result, override)
            return _result
