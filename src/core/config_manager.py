...
from src.config.constants import BATCH_SETTINGS, DEFAULT_PORTS, DEFAULTS, TIMEOUTS
...
class ConfigManager:
    ...
    def _load_default_sources(self):
        ...
        # Main config file
        config_paths = ["data/default_config.json", "config.json", "config.yaml", "settings.json"]
        ...
        for config_path in config_paths:
            ...
            if full_path.exists():
                ...
                self.add_source(ConfigSource(path=str(full_path), format=format_type, required=True, priority=50))
                break

        # Local override file (lower priority)
        local_config = self._base_dir / "config.local.json"
        if local_config.exists():
            self.add_source(ConfigSource(path=str(local_config), format=ConfigFormat.JSON, required=False, priority=75))
