...
class ConnectionPoolManager:
    ...
    def _create_session(self,
                       ...
                       status_forcelist: list) -> requests.Session:
        """
        새로운 세션 생성 및 구성
        """
        ...
        for identifier, session in self._sessions.items():
            ...
            if hasattr(adapter, 'poolmanager') and adapter.poolmanager:
                pool_stats = {
                    ...
                    'num_requests': adapter.poolmanager.num_requests if hasattr(adapter.poolmanager, 'num_requests') else 0,
                    'num_connections_dropped': adapter.poolmanager.num_connections_dropped if hasattr(adapter.poolmanager, 'num_connections_dropped') else 0
                }
            else:
                pool_stats = {
                    'num_connections': 0,
