...
class FortiGateAPIValidator:
    ...
    async def run_all_validations(self, 
        ...
        logger.info(f"Validation completed in {total_time:.2f}s - {summary['total_tests']} tests, {summary['passed']} passed, {summary['failed']} failed")
    ...
    async def _test_throughput(self):
        ...
        try:
            # 짧은 시간 내 많은 요청 수행
            tasks = [
                self.api_client.get_system_status() 
    ...
    async def _test_api_security(self):
        ...
        if hasattr(self.api_client, 'api_key') and self.api_client.api_key:
            ...
            # API 키 복잡성 검사 (간단한 버전)
            if any(c.isupper() for c in api_key) and any(c.islower() for c in api_key) and any(c.isdigit() for c in api_key):
    ...
    async def _test_security_profiles(self):
        ...
        available_profiles = sum(
            count for count in profile_results.values() 
...
async def validate_fortigate_api(api_client: AdvancedFortiGateAPI,
                               test_categories: List[str] = None,
                               save_results: str = None) -> Dict[str, Any]:
    """
