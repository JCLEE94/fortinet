...
class DashboardDataCollector:
    ...
    def _is_fortimanager_available(self) -> bool:
        ...
        _fm_client = self.api_manager.get_fortimanager_client()
            if not fm_client:
            ...
            # 장치 목록 가져오기
                ...
                for device in devices:
                    try:
                        ...
                        # 장치 상태 확인
                            device_status = fm_client.get_device_status(
                                                                        device_id)
                            if device_status and device_status.get(
                                                                   "status") == "online":
                                online_count += 1
                        ...
                        # 정책 수 가져오기
                                    policies = fm_client.get_policies(
                                                                      device_id)
                            if policies:
            ...
            try:
                events = fm_client.get_security_events(limit=100)
                    ...
                    recent_time = datetime.now() - timedelta(hours=24)
                        recent_events = [
                            event for event in events if datetime.fromisoformat(
                                                                                                            event.get("timestamp",
                                                                                                                "")) > recent_tim
                            )
                            ]
                ...
                try:
                    ...
                    system_stats = fg_client.get_system_status()
                        if system_stats:
                            ...
                            total_sessions += system_stats.get(
                                                               "session_count", 0)
                        ...
                        if interfaces:
                            for interface in interfaces:
                                ...
                                total_bandwidth_in += stats_data.get(
                                                                     "rx_bytes", 0) / (1024 * 1024)
                                # MB
                                total_bandwidth_out += stats_data.get(
                                                                      "tx_bytes", 0) / (1024 * 1024)

                                except Exception as e:
                                    logger.warning(
                                                   f"FortiGate {device_id} 데이터 수집 실패: {e}")
                        continue
