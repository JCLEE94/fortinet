    ...
    def _collect_data(self) -> Optional[Dict]:
        ...
        try:
            ...
            for endpoint, metrics in self.endpoint_metrics.items():
                if metrics['total'] > 0:
                    recent_calls = [call for call in metrics['calls'] 
            ...
            if total_response_times:
                ...
                overall_stats['overall_error_rate'] = (overall_stats['total_errors'] / overall_stats['total_requests']) * 100
        ...
        with self._lock:
            ...
            for endpoint in all_endpoints:
                ...
                if stats:
                    ...
                    cutoff = datetime.now() - timedelta(hours=hours)
                    recent_times = [
                        m['response_time'] for m in self.metrics[endpoint]
                        if datetime.fromisoformat(m['timestamp']) > cutoff
                    ]
        ...
        for ep in slow_endpoints:
            if ep['avg_response_time'] > self.thresholds['response_time_warning']:
                suggestions.append({
                    ...
                    'priority': 'high' if ep['avg_response_time'] > self.thresholds['response_time_critical'] else 'medium',
    ...
    def _analyze_performance(self, endpoint: str, metric: Dict):
        ...
        if 'response_time_critical' in self.thresholds:
            ...
            if response_time > critical_val:
                self._notify_listeners('slow_response', {
                    'endpoint': endpoint,
                    'response_time': response_time,
                    'severity': 'critical'
                })
        ...
        # 오류 감지
        if not metric['success']:
            self._notify_listeners('api_error', {
                'endpoint': endpoint,
