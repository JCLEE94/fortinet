    ...
    def __init__(self, collection_interval=None):
        ...
        # API 성능 특화 데이터 구조 (누락된 속성들 추가)
            ...
            self.metrics = defaultdict(
                                       lambda: deque(maxlen=1000))  # 수정: 누락된 속성
        ...
        try:
            ...
            total_response_times = []
                ...
                for endpoint, metrics in self.endpoint_metrics.items():
                    ...
                    if recent_calls:
                        response_times = [call["response_time"]
                        ...
                        overall_stats["total_requests"] += len(recent_calls)
                            ...
                            if total_response_times:
                                overall_stats["avg_response_time"] = sum(
                                                                         total_response_times) / len(total_response_times)
        ...
        @functools.wraps(func)
            ...
            try:
                ...
                # Flask response 객체인 경우
                    if hasattr(_result, "status_code"):
                        status_code = _result.status_code
                    response_size = len(_result.get_data()) if hasattr(_result, "get_data") else 0
                        else:
                            status_code = 200
                    response_size = len(str(_result)) if _result else 0
