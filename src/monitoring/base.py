        ...
        if self.is_running and self.is_paused:
            ...
            self._pause_event.clear()
               self.logger.info(f"{self.name} 모니터링 재개됨")
                ...
                def add_listener(
                                 self, callback: Callable[[str, Dict], None]) -> bool:
                    """
        리스너 추가
        ...
        with self._lock:
            stats = self.stats.copy()
               ...
               if stats["total_collections"] > 0:
                    stats["success_rate"] = stats["successful_collections"] / \
                        stats["total_collections"] * 100
                    stats["error_rate"] = stats["failed_collections"] / \
                        stats["total_collections"] * 100
                    else:
                   ...
                   data = self._collect_data()
                    ...
                    # 리스너들에게 알림
                        self._notify_listeners(
                                               "data_collected", processed_data or data)
        ...
        with self._lock:
            self.stats["total_collections"] += 1
               self.stats["last_collection_time"] = datetime.now().isoformat()
                ...
                self.stats["average_collection_time"] = total_time / \
