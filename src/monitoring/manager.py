...
logger = logging.getLogger(__name__)
    ...
    def get_events(self, hours: int = 1, event_type: str = None) -> List[Dict]:
        ...
        cutoff = datetime.now() - timedelta(hours=hours)
            ...
            with self._lock:
                filtered_events = (
                                   [event for event in self.events if datetime.fromisoformat(
                                                                              event["timestamp"]) > cutoff]
                )

                if event_type:
                    filtered_events = [
                        event for event in filtered_events if event.get("type") == event_type]
     ...
     reverse=True)
            ...
            # 결과를 캐시에 저장 (30초 TTL)
                self._event_cache.set(cache_key, _result)
    ...
    def __init__(self):
        ...
        try:
            with self._lock:
                ...
                # 인사이트 생성
                    integrated["insights"] = self._generate_insights(
                                                                     monitor_data)
        ...
        try:
            ...
            if "system_metrics" in data and "api_performance" in data:
                system_data = data["system_metrics"].get("data", {})
                    _api_data = data["api_performance"].get("data", {})

                    cpu_usage = system_data.get(
                                                "cpu", {}).get(
                                                    "usage_percent", 0)
                    memory_usage = system_data.get(
                                                   "memory", {}).get(
                                                       "usage_percent", 0)
            ...
            if "security_scanner" in data and "system_metrics" in data:
                security_data = data["security_scanner"].get("data", {})
                    ...
                    if vulnerabilities:
                        high_severity = [
                            v for v in vulnerabilities if v.get("severity") in [
                                        "high", "critical"]]
                        if high_severity:
        ...
        insights = []
            ...
            if "security_scanner" in data:
                security_data = data["security_scanner"].get("data", {})
                    ...
                    critical_vulns = scan_result.get(
        ...
        alerts = []
            ...
            # 중복 제거 및 우선순위 정렬
                ...
                def _deduplicate_alerts(
                                        self, alerts: List[Dict]) -> List[Dict]:
                    """중복 알림 제거"""
        seen = set()
            unique_alerts = []
