    ...
    def _init_api_config(self, prefix: str) -> APIConfig:
        ...
        # 환경변수에서 로드
            ...
            config.verify_ssl = os.getenv(
                                          f"{prefix}_VERIFY_SSL", "false").lower() == "true"
    ...
    def _load_from_json(self):
        ...
        try:
            ...
            self._update_api_from_json(
                                       "fortimanager", data.get(
                                           "fortimanager", {}))
               self._update_api_from_json(
                                          "fortigate", data.get("fortigate", {}))
                self._update_api_from_json(
                                           "fortianalyzer", data.get(
                                               "fortianalyzer", {}))

                except Exception as e:
                    print(f"JSON 설정 파일 로드 오류: {e}")

                def _update_api_from_json(
                                          self, service: str, json_config: Dict[str, Any]):
                    """JSON에서 API 설정 업데이트"""
        if not json_config:
            return
        ...
        # API 토큰 필드명 통일 (api_key → api_token)
                token_field = json_config.get(
                                              "api_token") or json_config.get("api_key", "")
        ...
        # API 설정 검증
                for service_name in ["fortimanager",
