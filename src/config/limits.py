    ...
    "top_items": int(os.getenv("DISPLAY_TOP_ITEMS", "10")),
                     "max_table_rows": int(os.getenv("MAX_TABLE_ROWS", "100")),
                                           "preview_length": int(os.getenv("PREVIEW_LENGTH", "200")),
                                                                 "truncate_length": int(
                                                                                                                                                         os.getenv("TRUNCATE_LENGTH",
                                                                                                                                                             "50"))
                                                                 )
                                                                                                                                                      "max_log_entries": int(
                                                                                                                                                                             os.getenv("MAX_LOG_ENTRIES",
                                                                                                                                                                                 "1000"))
                                                                           )
                                                                                                                                                                "pagination_size": int(
                                                                                                                                                                                       os.getenv("PAGINATION_SIZE",
                                                                                                                                                                                           "20"))
                                                                                )
    ...
    "default": int(os.getenv("DEFAULT_TIMEOUT", "30")),
                   "api_request": int(os.getenv("API_REQUEST_TIMEOUT", "60")),
                                      "health_check": int(os.getenv("HEALTH_CHECK_TIMEOUT", "5")),
                                                          "internet_check": int(
                                                                                                                                          os.getenv("INTERNET_CHECK_TIMEOUT",
                                                                                                                                              "5"))
                                                          )
                                                                                                                                          "long_operation": int(
                                                                                                                                                                os.getenv("LONG_OPERATION_TIMEOUT",
                                                                                                                                                                    "300"))
                                                                     )
                                                                                                                                                        "session": int(
                                      ...
                                      "business_hours_end": os.getenv("BUSINESS_HOURS_END", "18:00"),
                                                                      "business_timezone": os.getenv(
                                                                                                     "BUSINESS_TIMEZONE",
                                                                                                                                              ...
                                                                                                                                              "retention_days": int(
                                                                                                                                                                    os.getenv("METRICS_RETENTION_DAYS",
                                                                                                                                                                              "30")
                                                                       )
...
def is_within_limit(value: int, limit_type: str, limit_category: str = "SIZE_LIMITS") -> bool:
    ...
    limits = globals().get(limit_category, {})
        limit = limits.get(limit_type, float("in"))
        return value <= limit
