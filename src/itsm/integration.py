...
logger = get_logger(__name__)
    ...
    def __init__(self, itsm_base_url=None, api_key=None):
        ...
        try:
            src_ip = request_data.get("source_ip", "")
                ...
                _service = request_data.get("service", "")
    ...
    def _generate_implementation_steps(
                                       self, recommended_firewalls: List[Dict[str, Any]], request_data: Dict[str, Any]
                                           ) -> List[Dict[str, Any]]:
        """구현 단계 생성"""
        steps = []
                ...
                "title": f"방화벽 정책 요청: {request_data.get('source_ip')} -> {request_data.get('destination_ip')}",
                                                        "description": self._generate_ticket_description(
                                                               ...
                                                               "requester": request_data.get("requester", ""),
                                                                                             "department": request_data.get(
                                                                                                                            "department",
                                                                                                                                                                                                                             ""
                                                                                             )
                                                                                                  "requested_date": datetime.now().isoformat(),
                                                                                                                                 "firewall_details": analysis_result.get(
                                                                                                                                                                         "recommended_firewalls",
                                                                                                                                                                             [])
                                                                   )
                                                                                                                                "implementation_steps": analysis_result.get(
                                                                                                                                                                            "implementation_steps",
                                                                                                                                                                                [])
                                                                        )
                                                                                                                                        "approval_required": analysis_result.get(
                                                                                                                                                                                 "approval_required",
                                                                                                                                                                                     {}).get("required",
                                                                                                                                                                                             False)
                                                                        )
