...
class PolicyMapper:
    ...
    def map_itsm_to_fortigate_policy(self, itsm_request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            mapping_result = {
                ...
                'validation_tests': self._generate_validation_tests(source_networks, destination_networks, service_info),
    ...
    def _select_target_fortigates(self, source_zone: str, destination_zone: str) -> List[str]:
        ...
        if source_zone == 'internal' and destination_zone == 'external':
            ...
            # 같은 존 내 트래픽
            zone_firewalls = [
                fw_id for fw_id, fw_info in self.fortigate_devices.items()
    ...
    def _generate_fortigate_policy(self, fw_id: str, itsm_request: Dict, source_networks: List[str],
                                 destination_networks: List[str], source_zone: str, destination_zone: str,
                                 service_info: Dict, action: str) -> Dict[str, Any]:
        """FortiGate 정책 생성"""
        ...
        cli_commands = self._generate_cli_commands(
            policy_name, src_zone, dst_zone, src_addresses, dst_addresses, service_objects, action
        )
    ...
    def _generate_cli_commands(self, policy_name: str, src_zone: str, dst_zone: str,
                             src_addresses: List[Dict], dst_addresses: List[Dict],
                             service_objects: List[Dict], action: str) -> List[str]:
        """FortiGate CLI 명령 생성"""
        commands = []
    ...
    def _generate_validation_tests(self, source_networks: List[str], destination_networks: List[str],
                                 service_info: Dict) -> List[Dict[str, str]]:
        """검증 테스트 생성"""
        tests = []
