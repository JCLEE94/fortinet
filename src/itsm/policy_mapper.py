...
class PolicyMapper:
    ...
    def __init__(self):
        ...
        self.network_zones = {
            ...
            "external": {"name": "외부망", "networks": ["0.0.0.0/0"], "fortigate_zone": "wan", "security_level": "low"},
            "branch": {
                "name": "지사",
    ...
    def _process_service_info(self, port: str, protocol: str) -> Dict[str, Any]:
        """포트 및 서비스 정보 처리"""
        service_info = {"ports": [], "protocol": protocol, "service_names": [], "custom_services": []}
    ...
    def _create_service_objects(self, service_info: Dict, prefix: str) -> List[Dict[str, Any]]:
        ...
        for i, port in enumerate(service_info["ports"]):
            ...
            service_objects.append(
                {"name": svc_name, "protocol": service_info["protocol"], "port_range": port, "type": "custom"}
            )
    ...
    def _assess_security_impact(self, source_zone: str, destination_zone: str, service_info: Dict) -> Dict[str, Any]:
        """보안 영향 평가"""
        impact = {"risk_level": "medium", "concerns": [], "recommendations": []}
    ...
    def _generate_validation_tests(
        self, source_networks: List[str], destination_networks: List[str], service_info: Dict
    ) -> List[Dict[str, str]]:
        """검증 테스트 생성"""
        tests = []
