...
class PolicyAutomationEngine:
    ...
    def _initialize_default_zones(self):
        ...
        default_zones = [
            ...
            NetworkZone(
                name="external", cidr="0.0.0.0/0", description="외부 네트워크", security_level=1, allowed_outbound=[]
            ),
            NetworkZone(
                name="guest",
                cidr="10.0.0.0/16",
    ...
    def _determine_target_firewalls(
        self, src_zone: NetworkZone, dst_zone: NetworkZone, request: FirewallPolicyRequest
    ) -> List[FirewallDevice]:
        """경로상 필요한 방화벽 장치들 결정"""
        target_firewalls = []
    ...
    def _calculate_deployment_order(
        self, firewalls: List[FirewallDevice], src_zone: NetworkZone, dst_zone: NetworkZone
    ) -> List[str]:
    ...
    def _get_security_profiles(
        ...
        # 내부에서 외부로 나가는 트래픽
            ...
            profiles.update({"antivirus": "default", "webfilter": "default", "application-list": "default"})
    ...
    def _assess_security_risk(
        self, request: FirewallPolicyRequest, src_zone: NetworkZone, dst_zone: NetworkZone
    ) -> str:
        """보안 리스크 평가"""
        risk_factors = []
        risk_score = 0
    ...
    def _should_auto_approve(
        self, request: FirewallPolicyRequest, src_zone: NetworkZone, dst_zone: NetworkZone, risk_assessment: str
    ) -> bool:
        """자동 승인 여부 결정"""
        # 높은 리스크는 자동 승인 안함
        ...
        for request in requests:
            try:
                ...
                if report.result == DeploymentResult.SUCCESS:
                    ...
                    await connector.update_ticket_status(
                        request.ticket_id, "in_progress", f"Deployment failed: {'; '.join(report.error_messages)}"
                    )
