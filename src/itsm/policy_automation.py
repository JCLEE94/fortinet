...
from .external_connector import FirewallPolicyRequest, ExternalITSMConnector
...
class PolicyAutomationEngine:
    ...
    def analyze_firewall_request(self, request: FirewallPolicyRequest) -> PolicyDeploymentPlan:
        ...
        logger.info(f"Deployment plan created for {request.ticket_id}: {len(target_firewalls)} firewalls, auto_approve={auto_approve}")
        return plan
    
    def _determine_target_firewalls(self, src_zone: NetworkZone, dst_zone: NetworkZone, request: FirewallPolicyRequest) -> List[FirewallDevice]:
        """경로상 필요한 방화벽 장치들 결정"""
        target_firewalls = []
    ...
    def _calculate_deployment_order(self, firewalls: List[FirewallDevice], src_zone: NetworkZone, dst_zone: NetworkZone) -> List[str]:
    ...
    def _generate_policy_rules(self, request: FirewallPolicyRequest, src_zone: NetworkZone, dst_zone: NetworkZone, firewalls: List[FirewallDevice]) -> List[Dict]:
        """예상 정책 규칙 생성"""
        rules = []
    ...
    def _get_security_profiles(self, src_zone: NetworkZone, dst_zone: NetworkZone, request: FirewallPolicyRequest) -> Dict:
        """보안 프로파일 결정"""
        profiles = {}
    ...
    def _assess_security_risk(self, request: FirewallPolicyRequest, src_zone: NetworkZone, dst_zone: NetworkZone) -> str:
        """보안 리스크 평가"""
        risk_factors = []
        risk_score = 0
    ...
    def _should_auto_approve(self, request: FirewallPolicyRequest, src_zone: NetworkZone, dst_zone: NetworkZone, risk_assessment: str) -> bool:
        """자동 승인 여부 결정"""
        # 높은 리스크는 자동 승인 안함
    ...
    async def _deploy_direct_fortigate(self, firewall: FirewallDevice, rule: Dict) -> Dict:
        ...
        try:
            ...
            if success:
                ...
                return {
                    'success': False,
                    'error': 'FortiGate API call failed'
                }
