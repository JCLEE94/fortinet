   ...
   class ITSMAutomationService:
                ...
                connector_config = ITSMConfig(
                        ...
                        username=itsm_config.get("username", ""),
                                             password=itsm_config.get("password", ""),
                                                 api_token=itsm_config.get(
                                              "api_token",
                                                      ""
                                                      )
                    custom_headers=itsm_config.get(
                                                   "custom_headers",
                                                      {}
                                                                               )
                    poll_interval=itsm_config.get(
                                                  "poll_interval",
                                                      300
                                                      )
                    )
    ...
    async def _send_notifications(self, reports: List[DeploymentReport]):
        ...
        failed_reports = [r for r in reports if r.result.value != "success"]
           ...
           if failed_reports:
                logger.warning(
                               f"Found {len(failed_reports)} failed deployments")
            # 여기서 웹훅, 이메일 등으로 알림 전송
            # 실제 구현은 설정에 따라 다름

                async def manual_process(
                                         self, since_hours: int = 1) -> List[DeploymentReport]:
        """수동 처리 (즉시 실행)"""
           logger.info(f"Starting manual processing for last {since_hours} hours")
            ...
            # 지정된 시간 이후의 요청 처리
                ...
                for request in requests:
                    plan = self.automation_engine.analyze_firewall_request(
                                                                           request)
        ...
        if self.service_stats["start_time"]:
            uptime = (
                      datetime.now() -
     self.service_stats["start_time"]).total_seconds()
