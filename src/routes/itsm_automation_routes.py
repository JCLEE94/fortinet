...
logger = get_logger(__name__)
   ...
   itsm_automation_bp = Blueprint(
                                  "itsm_automation",
                                      __name__,
     url_prefix="/api/itsm/automation")
...
@itsm_automation_bp.route("/status", methods=["GET"])
   @rate_limit(max_requests=30, _window=60)
    ...
    def get_automation_status():
        """자동화 서비스 상태 조회"""
    try:
        _service = get_automation_service()
    ...
    @rate_limit(max_requests=5, _window=300)  # 5분에 5번만 허용
    @async_route
async def start_automation_service():
    ...
    try:
        ...
        # 비동기로 서비스 시작 (백그라운드에서 실행)
            ...
            return jsonify(
                           {"status": "success", "message": "Automation service started"})
...
async def stop_automation_service():
    ...
    try:
        _service = get_automation_service()
           ...
           if not service.is_running:
                return jsonify(
                               {"status": "warning", "message": "Service is not running"})

                await service.stop_service()

            return jsonify(
                           {"status": "success", "message": "Automation service stopped"})
    ...
    @rate_limit(max_requests=10, _window=300)  # 5분에 10번만 허용
    @async_route
async def manual_process():
    ...
    @rate_limit(max_requests=10, _window=300)
    @async_route
async def test_itsm_connection():
    """ITSM 연결 테스트"""
    try:
        _service = get_automation_service()
           _result = await service.test_itsm_connection()
    ...
    def simulate_policy_request():
        ...
        sim_request = FirewallPolicyRequest(
                ...
                port=data["port"],
                            ...
                            _description=data.get("description", "Simulation request"),
                                  business_justification=data.get(
                                            "justification",
                                                                              "Testing automation"
                                                      )
