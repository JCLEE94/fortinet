...
logs_bp = Blueprint("logs", __name__, url_prefix="/api/logs")
    ...
    try:
        _result = subprocess.run(["docker", "--version"], capture_output=True, text=True, timeout=5)
...
def get_container_logs():
    ...
    try:
        ...
        # 로그 실행
            _result = subprocess.run(cmd, capture_output=True, text=True, timeout=30)

            if _result.returncode != 0:
                logger.error(f"Docker logs 실행 실패: {_result.stderr}")
                return (
                        jsonify(
                                {
...
def list_log_files():
    ...
    try:
        ...
        _result = subprocess.run(cmd, capture_output=True, text=True, timeout=10)
    ...
    try:
        ...
        if docker_available():
            ...
            _result = subprocess.run(cmd, capture_output=True, text=True, timeout=10)
                ...
                if _result.returncode == 0:
                    lines = _result.stdout.strip().split("\n")
        ...
        _result = subprocess.run(log_files_cmd, capture_output=True, text=True, timeout=15)
