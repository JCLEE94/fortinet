...
def search_logs():
    ...
    try:
        ...
        if result.returncode != 0:
            return jsonify({
                'error': 'Failed to read logs for search',
                'matches': []
            }), 500
    ...
    try:
        ...
        if docker_available():
            cmd = ['docker', 'stats', '--no-stream', '--format', 
                   'table {{.Container}}\t{{.CPUPerc}}\t{{.MemUsage}}\t{{.MemPerc}}', 'fortinet']
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=10)
        ...
        # 로그 파일 통계
        log_files_cmd = ['docker', 'exec', 'fortinet', 'find', '/app/logs', '-name', '*.log', '-exec', 'wc', '-l', '{}', '+']
