   ...
   compliance_bp = Blueprint("compliance", __name__, url_prefix="/compliance")
    ...
    try:
        data = request.get_json()
           ...
           if not data:
                return jsonify(
                               {"error": "Compliance check parameters are required"}), 400
            ...
            if not devices:
                return jsonify(
                               {"error": "At least one device is required"}), 400
    ...
    try:
        data = request.get_json()
           ...
           if not data:
                return jsonify(
                               {"error": "Remediation parameters are required"}), 400
            ...
            if not devices or not violations:
                return jsonify(
                               {"error": "Devices and violations are required"}), 400
    ...
    try:
        ...
        # FortiManager 고급 허브 사용
            ...
            return jsonify(
                           {"schedules": schedules or [], "mode": "production"})
    ...
    try:
        data = request.get_json()
           ...
           if not data:
                return jsonify(
                               {"error": "Schedule parameters are required"}), 400
        ...
        if schedule_result:
            ...
            return jsonify(
                           {"error": "Failed to schedule compliance check"}), 500
