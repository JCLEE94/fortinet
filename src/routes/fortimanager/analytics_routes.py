...
logger = setup_logger("analytics_routes")
   analytics_bp = Blueprint("analytics", __name__, url_prefix="/analytics")
    ...
    @rate_limit(max_requests=5, _window=300)
    ...
    try:
        data = request.get_json()
           ...
           if not data:
                return jsonify(
                               {"error": "Capture configuration is required"}), 400
        ...
        api_manager = get_api_manager()
           _fm_client = api_manager.get_fortimanager_client()
            ...
            if not fm_client:
                return jsonify(
                               {"error": "FortiManager client not available"}), 503

                _result = fm_client.start_packet_capture(
        ...
        api_manager = get_api_manager()
           _fm_client = api_manager.get_fortimanager_client()
            ...
            if not fm_client:
                ...
                _result = fm_client.stop_packet_capture(capture_id)
            ...
            @analytics_bp.route("/packet-capture/results/<capture_id>",
                                methods=["GET"])
    @cached(ttl=60)
        ...
        if is_test_mode():
            dummy_generator = get_dummy_generator()
               capture_results = dummy_generator.generate_capture_results(capture_id)
                return jsonify({"capture_id": capture_id,
                               "results": capture_results, "mode": "test"})
            ...
            if results:
                return jsonify({"capture_id": capture_id,
                               "results": results, "mode": "production"})
                else:
    ...
    @rate_limit(max_requests=20, _window=300)
    ...
    try:
        data = request.get_json()
           ...
           if not data:
                return jsonify(
                               {"error": "Analysis parameters are required"}), 400
    ...
    @rate_limit(max_requests=15, _window=300)
    ...
    try:
        data = request.get_json()
           ...
           if not data:
                return jsonify(
                               {"error": "Detection parameters are required"}), 400
        ...
        # 비동기 이상 탐지 수행
            anomalies_result = await hub.analytics_engine.detect_anomalies(
                                                                           devices=devices, detection_types=detection_types, sensitivity=sensitivity
                )
        ...
        logger.error(f"이상 탐지 중 오류: {str(e)}")
           return jsonify({"error": str(e)}), 500
            ...
            @analytics_bp.route("/advanced/capacity-planning",
                                methods=["POST"])
    @rate_limit(max_requests=10, _window=600)
    ...
    try:
        data = request.get_json()
           ...
           if not data:
                return jsonify(
                               {"error": "Capacity analysis parameters are required"}), 400
        ...
        # 비동기 용량 분석 수행
            capacity_result = await hub.analytics_engine.predict_capacity(
                                                                          devices=devices, forecast_period=forecast_period, resources=resources
                )
    ...
    @rate_limit(max_requests=25, _window=300)
    ...
    try:
        data = request.get_json()
           ...
           if not data:
                return jsonify(
                               {"error": "Threat detection parameters are required"}), 400

                devices = data.get("devices", [])
            threat_types = data.get(
                                    "threat_types", [
                                        "malware", "intrusion", "botnet"])
        ...
        logger.error(f"위협 탐지 중 오류: {str(e)}")
           return jsonify({"error": str(e)}), 500
            ...
            @analytics_bp.route("/security/incident-response",
                                methods=["POST"])
    @rate_limit(max_requests=5, _window=300)
