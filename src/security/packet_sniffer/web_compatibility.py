...
logger = logging.getLogger(__name__)
   ...
   class WebCompatibilityInterface:
    ...
    def start_capture(
        ...
        try:
            ...
            if hasattr(self.packet_sniffer,
                       "api_client") and self.packet_sniffer.api_client:
                try:
            ...
            # 세션 시작
            _result = self.packet_sniffer.start_capture_session(params)
               ...
               if result["success"]:
                ...
                # 통계 업데이트
                    ...
                    self.statistics["last_capture"] = datetime.now(
                                                                   ).isoformat()
    ...
    def stop_capture(self, session_id: str) -> bool:
        ...
        """
        try:
            _result = self.packet_sniffer.stop_capture_session(session_id)

               if result["success"]:
                    logger.info(f"웹 캡처 중지 성공: {session_id}")
                    else:
                        logger.warning(
                                       f"웹 캡처 중지 실패: {session_id} - {_result.get('message', 'Unknown error')}")
    ...
    def get_captured_packets(self, session_id: str,
                             max_packets: int = 10) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if hasattr(self.packet_sniffer, "get_latest_packets"):
                _result = self.packet_sniffer.get_latest_packets(
        ...
        try:
            if (
                ...
                return {"state": "unknown",
                    "captured_packets": 0, "success": False}
            ...
            # 저장된 패킷 수 계산
               stored_count = 0
            if hasattr(self.packet_sniffer,
                       "stored_packets") and session_id in self.packet_sniffer.stored_packets:
                stored_count = len(
                                   self.packet_sniffer.stored_packets[session_id])
