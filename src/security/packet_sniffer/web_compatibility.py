...
class WebCompatibilityInterface:
    ...
    def start_capture(self, interface: str, filter_str: str = "", max_packets: int = 1000, timeout: int = 60) -> str:
        """
    ...
    def get_captured_packets(self, session_id: str, max_packets: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            # 세션이 유효한지 확인
            if not hasattr(self.packet_sniffer, 'active_sessions') or session_id not in self.packet_sniffer.active_sessions:
                ...
                try:
                    ...
                    for _ in range(max_packets):
                        try:
                            ...
                            # 패킷 저장 (나중에 조회할 수 있도록)
                            if (hasattr(self.packet_sniffer, 'stored_packets') and 
                                session_id in self.packet_sniffer.stored_packets):
    ...
    def get_capture_status(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            if not hasattr(self.packet_sniffer, 'active_sessions') or session_id not in self.packet_sniffer.active_sessions:
                return {'state': 'unknown', 'captured_packets': 0, 'success': False}
            ...
            return {'state': 'error', 'captured_packets': 0, 'success': False, 'error': str(e)}
    ...
    def get_available_interfaces(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not hasattr(self.packet_sniffer, 'api_client') or not self.packet_sniffer.api_client:
                ...
                # 기본 인터페이스 목록 반환
                return [
                    {'name': 'port1', 'type': 'physical', 'ip': '192.168.1.1/24', 'status': 'up'},
                    {'name': 'port2', 'type': 'physical', 'ip': '10.0.0.1/24', 'status': 'up'}
                ]
...
# 팩토리 함수
def create_web_compatibility_interface(packet_sniffer) -> WebCompatibilityInterface:
    """웹 호환성 인터페이스 인스턴스 생성"""
