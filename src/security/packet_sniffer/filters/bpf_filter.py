   ...
   class BPFFilter:
    ...
    def matches(self, packet_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            _result = self._evaluate_filter(self.compiled_filter, packet_info)
    ...
    def _tokenize(self, filter_string: str) -> List[str]:
        ...
        try:
            ...
            if condition.startswith(
                                    "src ") or condition.startswith("src host "):
                ip = condition.replace(
                                       "src host ", "").replace(
                                           "src ", "").strip()
                   return {"type": "src_host", "ip": ip}

            if condition.startswith(
                                    "dst ") or condition.startswith("dst host "):
                ip = condition.replace(
                                       "dst host ", "").replace(
                                           "dst ", "").strip()
    ...
    def _evaluate_filter(
                         self, filter_expr: Dict[str, Any], packet_info: Dict[str, Any]) -> bool:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = self.statistics.copy()
           ...
           if stats["total_packets"] > 0:
                stats["match_rate"] = stats["matched_packets"] / \
                    stats["total_packets"]
                else:
