...
logger = logging.getLogger(__name__)
    ...
    def matches(self, packet_info: Dict[str, Any],
        ...
        try:
            if self.rule_type == "time_based":
                ...
                return self._match_ml(packet_info, context)
                        ...
                        return False, {
                            "error": f"Unknown rule type: {self.rule_type}"}
        ...
        try:
            ...
            # 시간 범위 체크
                if "time_range" in params:
                    ...
                    if start_time and current_time.time() < datetime.strptime(
                                                        start_time, "%H:%M:%S").time():
                        return False, {}
                if end_time and current_time.time() > datetime.strptime(end_time,
                                                  "%H:%M:%S").time():
                    return False, {}
        ...
        try:
            ...
            model_type = params.get("model_type", "anomaly_detection")
                ...
                if model_type == "anomaly_detection":
                    return self._ml_anomaly_detection(
                                                      packet_info, params, context)
                ...
                return self._ml_clustering(packet_info, params, context)
                        ...
                        return False, {
                            "error": f"Unknown ML model type: {model_type}"}
    ...
    def _extract_metric_value(
                              self, packet_info: Dict[str, Any], metric: str) -> Optional[Union[int, float]]:
        """패킷에서 메트릭 값 추출"""
        try:
    ...
    def _check_change_rate(
        ...
        try:
            metric = change_params.get("metric")
                ...
                _window = change_params.get("_window", 60)  # 60초 윈도우
            ...
            # 최근 값들의 평균 계산
            recent_avg = sum(
                             historical__values[-10:]) / len(historical_values[-10:])
    ...
    def _match_sequence_pattern(
        ...
        try:
            pattern = pattern_params.get("pattern", [])
                _window = pattern_params.get("_window", 10)
    ...
    def _match_network_pattern(
                               self, packet_info: Dict[str, Any], network_params: Dict[str, Any]) -> bool:
        """네트워크 패턴 매칭"""
        try:
    ...
    def _detect_port_scan(
                          self, packet_info: Dict[str, Any], params: Dict[str, Any]) -> bool:
        """포트 스캔 탐지"""
