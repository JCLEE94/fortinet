   ...
   class FilterRule:
    ...
    def matches(self, packet_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # 연산자별 비교
            if self.operator == "eq":
                _result = field_value == self.value
                ...
                _result = field_value in self.value
            elif self.operator == "contains":
                _result = str(self.value) in str(field_value)
                   elif self.operator == "regex":
                _result = bool(re.search(str(self.value), str(field_value)))
                   elif self.operator == "subnet":
                    # IP 서브넷 매칭
                try:
                    network = ipaddress.ip_network(self.value, strict=False)
                       ip = ipaddress.ip_address(field_value)
                        _result = ip in network
                except Exception:
                    _result = False
            else:
    ...
    def clear_rules(self):
        ...
        self.rules.clear()
           logger.info("모든 필터 규칙 제거됨")
            ...
            def filter_packet(
                              self, packet_info: Dict[str, Any]) -> Dict[str, Any]:
                """
        패킷 필터링
        ...
        for ip in ip_list:
            try:
                if "/" in ip:
                    ...
                    def add_port_filter(
                                        self, ports: List[int], action: str = "allow"):
                        """포트 필터 추가"""
        for port in ports:
            self.add_rule("src_port", "eq", port, action)
               self.add_rule("dst_port", "eq", port, action)
                ...
                def add_protocol_filter(
                                        self, protocols: List[str], action: str = "allow"):
                    """프로토콜 필터 추가"""
