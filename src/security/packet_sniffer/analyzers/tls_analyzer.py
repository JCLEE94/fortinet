...
logger = logging.getLogger(__name__)
    ...
    def analyze(self, packet_data: bytes,
        ...
        try:
            ...
            # 연결 정보 업데이트
                            connection_key = (f"{packet_info.get('src_ip')}:{packet_info.get('src_"
    ...
    def _is_tls_packet(self, packet_data: bytes,
                       packet_info: Dict[str, Any]) -> bool:
        """TLS 패킷인지 확인"""
        # 포트 기반 확인
        if packet_info.get("dst_port") in [443, 993, 995, 636] or packet_info.get(
                           "src_port") in [443, 993, 995, 636]:
            return True

        # TLS 레코드 헤더 확인
        if len(packet_data) >= 5:
            content_type = packet_data[0]
                _version = struct.unpack(">H", packet_data[1:3])[0]
        ...
        while offset < len(data) - 5:
            try:
                ...
                content_type = data[offset]
                    _version = struct.unpack(
                                             ">H", data[offset + 1: offset + 3])[0]
                    length = struct.unpack(
                                           ">H", data[offset + 3: offset + 5])[0]
    ...
    def _parse_client_hello(self, data: bytes) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            session_id_length = data[offset]
                offset += 1
            session_id = data[offset: offset +
     session_id_length] if session_id_length > 0 else b""
