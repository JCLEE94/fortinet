    ...
    def __init__(self):
        ...
        try:
            ...
            # 애플리케이션 분석기 (SSH, MQTT 등)
                ...
                def register_analyzer(self, name: str,
                                      analyzer: BaseProtocolAnalyzer) -> None:
                    """분석기 등록"""
        self.analyzers[name] = analyzer
            ...
            def perform_deep_packet_inspection(
                                               self, packet: PacketInfo) -> Dict[str, Any]:
        ...
        try:
            ...
            # 3단계: 포트 기반 프로토콜 추정
                    self._analyze_port_based_protocols(
            ...
            # 7단계: 신뢰도 계산
                confidence = self._calculate_protocol_confidence(
                                                                 inspection_result)
                inspection_result["confidence"] = confidence

            # 캐시에 저장
                _result = ProtocolAnalysisResult(
                                                 protocol=inspection_result.get(
                                                                                "protocol", "Unknown"),
                                                                                    confidence=confidence,
                                                                                                                                                                details=inspection_result.get(
                                                                                                                                                                                              "details",
                                                                                                                                                                                                  {})
                                                                                )
                                                                                 flags = inspection_result.get(
                                                                                                               "flags",
                                                                                                                                                                                                  {}
                                                                                 )
                                                                                                                                                                    hierarchy = inspection_result.get(
                                                                                                                                                                                                      "hierarchy",
                                                                                                                                                                                                        []
                                                                                  )
                                                                                                                                                                                    security_flags = inspection_result.get(
                                                                                                                                                                                                                           "security_flags",
                                                                                                                                                                                                                             {}
                                                                                          )
                                                                                                                                                                                          anomalies = inspection_result.get(
                                                                                                                                                                                                                            "anomalies",
                                                                                                                                                                                                                              []
                                                                                             )
                                                                                             )
                ...
                "cache_hits": self.stats["cache_hits"],
                    "cache_hit_rate": self.stats["cache_hits"] / max(self.stats["total_analyzed"], 1) * 100,
                                                                     "analysis_errors": self.stats["analysis_errors"],
                                                                         "error_rate": self.stats["analysis_errors"] / max(
    ...
    def clear_cache(self) -> None:
        ...
        results = []
            for packet in packets:
                _result = self.perform_deep_packet_inspection(packet)
