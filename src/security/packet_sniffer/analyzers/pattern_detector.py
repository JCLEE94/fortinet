...
class PatternDetector:
    ...
    def detect_patterns(self, packets: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"패턴 탐지 완료: 총 {len(packets)}개 패킷에서 "
    ...
    def _detect_periodic_patterns(self, sorted_packets: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 주기성 판단 (편차가 작은 경우)
            if max_interval > 0 and (max_interval - min_interval) / avg_interval < self.thresholds['periodic_variance_ratio']:
                # 주기적 패킷 그룹 식별
                periodic_groups = defaultdict(lambda: {
                    'count': 0,
                    'protocol': '',
                    'src_ip': '',
                    'dst_ip': '',
                ...
                if time_diff <= burst_window:
                    ...
                    if len(src_ips) == 1 and len(dst_ips) == 1:
                        ...
                        for j in range(i + min_packets, len(sorted_packets)):
                            ...
                            if (next_time - start_packet.get('timestamp', 0) <= burst_window and
                                next_packet.get('src_ip') == burst['src_ip'] and
                                next_packet.get('dst_ip') == burst['dst_ip']):
                                extended_count += 1
                        ...
                        burst['intensity'] = extended_count / burst['duration'] if burst['duration'] > 0 else float('inf')
                        
                        # 중복 제거 (유사한 시간대의 버스트)
                        is_duplicate = False
                        for existing_burst in burst_patterns:
                            if (abs(existing_burst['start_time'] - burst['start_time']) < 1.0 and
                                existing_burst['src_ip'] == burst['src_ip'] and
                                existing_burst['dst_ip'] == burst['dst_ip']):
                                is_duplicate = True
                                break
    ...
    def _detect_port_scans(self, sorted_packets: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for scan_key, scan in port_scans.items():
                ...
                if (port_count >= self.thresholds['scan_min_ports'] and
                    duration <= self.thresholds['scan_max_duration']):
    ...
    def _detect_targeted_patterns(self, sorted_packets: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for dst_ip, target in target_analysis.items():
                ...
                if (packet_ratio > self.thresholds['targeted_packet_ratio'] and
                    source_count >= self.thresholds['targeted_min_sources'] and
                    protocol_count >= self.thresholds['targeted_min_protocols']):
