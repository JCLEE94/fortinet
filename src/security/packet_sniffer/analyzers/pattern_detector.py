...
logger = logging.getLogger(__name__)
        ...
        try:
            ...
            # 각종 패턴 탐지 수행
                ...
                patterns["targeted"] = self._detect_targeted_patterns(
                                                                      sorted_packets)
                patterns["unusual"] = self._detect_unusual_patterns(
                                                                    sorted_packets)
    ...
    def _detect_periodic_patterns(
                                  self, sorted_packets: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """주기적 패턴 탐지"""
        try:
            ...
            # 시간 간격 계산
                ...
                for i in range(1, len(sorted_packets)):
                    ...
                    interval = current.get(
                                           "timestamp", 0) - prev.get("timestamp", 0)
            ...
            # 주기성 판단 (편차가 작은 경우)
                ...
                # 일정 개수 이상의 패킷이 있는 주기적 그룹 선택
                    ...
                    for group_key, group in periodic_groups.items():
                        ...
                        periodicity_score = group["count"] / \
                            expected_intervals if expected_intervals > 0 else 0
            ...
            try:
                ...
                # 시간 윈도우 확인
                    time_diff = end_packet.get(
                                               "timestamp", 0) - start_packet.get("timestamp", 0)

                    if time_diff <= burst_window:
                        # 같은 출발지/목적지 확인
                    window_packets = sorted_packets[i: i + min_packets]
                        src_ips = set(p.get("src_ip", "")
                                      for p in window_packets)
                        dst_ips = set(p.get("dst_ip", "")
                                      for p in window_packets)
                        protocols = set(p.get("protocol", "")
                                        for p in window_packets)
                    ...
                    # 동일한 엔드포인트 간의 버스트인지 확인
                                                                                                     ...
                                                                                                     "dst_ip": list(dst_ips)[0],
                                                                                                                    "protocols": list(protocols),
                                                                                                                           "packet_count": min_packets,
                                                                                                                               "duration": time_diff,
                                                                                                        "intensity": (min_packets / time_diff if time_diff > 0 else float("in")),
                                                                            }
                        ...
                        burst["packet_count"] = extended_count
                            burst["intensity"] = (
                                                  extended_count /
                                                      burst["duration"] if burst["duration"] > 0 else float(
                                                                                                            "in")
                                                  )
                        ...
                        for existing_burst in burst_patterns:
                            if (
                                abs(existing_burst["start_time"] -
                                    burst["start_time"]) < 1.0
        ...
        try:
            ...
            # 포트 스캔 조건 확인
                ...
                for scan_key, scan in port_scans.items():
                    ...
                    if port_count > = (
                                       self.thresholds["scan_min_ports"] and duration <= self.thresholds["scan_max_duration"]:
                    )
                    # 스캔 유형 분류
                    scan_type = "unknown"
                    ...
                    detected_scans.append(
                                        ...
                                        "dst_ip": scan["dst_ip"],
                                            "port_count": port_count,
                                                # 최대 20개 포트만 표시
                                                "ports": sorted(list(scan["ports"]))[:20],
                                                                "packet_count": scan["packet_count"],
                                                                    "duration": duration,
                                                    "start_time": scan["start_time"],
                                        "end_time": scan["end_time"],
                                            "scan_rate": (port_count / duration if duration > 0 else float("in")),
                                                          }
                                                  )
            ...
            # 호스트 스캔 조건 확인
                ...
                # 다수의 목적지 호스트를 짧은 시간에 스캔하는 경우
                    ...
                    detected_scans.append(
                                        ...
                                        # 최대 10개 호스트만 표시
                                        "dst_ips": sorted(list(scan["dst_ips"]))[:10],
                                                          "protocols": list(scan["protocols"]),
                                                              "packet_count": scan["packet_count"],
                                                                  "duration": duration,
                                                                      "start_time": scan["start_time"],
                                                          "end_time": scan["end_time"],
                                                              "scan_rate": (host_count / duration if duration > 0 else float("in")),
                                                              }
                                                  )
                        ...
                        targeted_patterns.append(
                                        ...
                                        "packet_ratio": packet_ratio,
                                            "source_count": source_count,
                                                # 최대 5개 발신지
                                                "sources": sorted(list(target["sources"]))[:5],
                                                                  "protocols": list(target["protocols"]),
                                                                      "total_bytes": target["total_bytes"],
                                                                  "duration": duration,
                                                                      "start_time": target["start_time"],
                                                          "end_time": target["end_time"],
                                                              "traffic_intensity": (target["count"] / duration if duration > 0 else float("in")),
                                                                      }
                                                          )
        ...
        try:
            ...
            # TCP 플래그 이상 분석
                    flag_anomalies = self._detect_flag_anomalies(
                                                                 sorted_packets)
                if flag_anomalies:
            ...
            logger.error(f"비정상 패턴 탐지 오류: {e}")
                ...
                def _detect_unusual_protocols(
                                              self, sorted_packets: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
                    """비정상 프로토콜 탐지"""
        common_protocols = {
            "TCP",
                "UDP",
            ...
            if max_gap > avg_gap * 10:
                return {
                    "type": "time_anomalies",
                        ...
                        "avg_gap": avg_gap,
                            "max_gap": max_gap,
                                "gap_ratio": max_gap / avg_gap if avg_gap > 0 else float("in"),
                                                                                         }
