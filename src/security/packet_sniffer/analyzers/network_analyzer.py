...
logger = logging.getLogger(__name__)
    ...
    def __init__(self):
        self.connections = defaultdict(dict)  # TCP 연결 추적
            self.flow_stats = defaultdict(
                                          lambda: {"packets": 0, "bytes": 0})  # 플로우 통계
    ...
    def _analyze_ip_layer(self, packet_data: bytes,
    ...
    def _analyze_ipv4(self, packet_data: bytes,
                      packet_info: Dict[str, Any]) -> Dict[str, Any]:
        """IPv4 헤더 분석"""
        try:
            if len(packet_data) < 20:
            ...
            logger.error(f"IPv4 분석 오류: {e}")
                ...
                def _analyze_ipv6(self, packet_data: bytes,
                                  packet_info: Dict[str, Any]) -> Dict[str, Any]:
                    """IPv6 헤더 분석"""
        try:
            ...
            version_class_label = struct.unpack(">I", packet_data[0:4])[0]
                _version = (version_class_label >> 28) & 0xF
            ...
            logger.error(f"IPv6 분석 오류: {e}")
                ...
                def _parse_ipv4_options(
                                        self, options_data: bytes) -> List[Dict[str, Any]]:
                    """IPv4 옵션 파싱"""
        options = []
            offset = 0
        ...
        while offset < len(options_data):
            if options_data[offset] == 0:  # End of Option List
                ...
                opt_type = options_data[offset]
                    opt_length = options_data[offset +
                        1] if offset + 1 < len(options_data) else 0

                    if opt_length < 2 or offset + \
                        opt_length > len(options_data):
                        break
    ...
    def _analyze_tcp(self, packet_data: bytes,
                     packet_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """TCP 세그먼트 분석"""
        try:
            # IP 헤더 길이 계산
        ...
        try:
            ...
            # TCP 상태 머신 업데이트
                ...
                connection["state"] = "SYN_RECEIVED"
                    elif "ACK" in flags and connection["state"] in ["SYN_SENT", "SYN_RECEIVED"]:
