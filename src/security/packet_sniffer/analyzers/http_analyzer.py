...
class HttpAnalyzer(BaseProtocolAnalyzer):
    ...
    def _is_error_page(self, content: str) -> bool:
        """에러 페이지 탐지"""
        error_indicators = [
            'stack trace', 'exception', 'error 500', 'internal server error',
            'sql error', 'database error', 'php error', 'python error',
    ...
    def _calculate_http_confidence(self, packet: PacketInfo, payload: str, 
