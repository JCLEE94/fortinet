...
logger = logging.getLogger(__name__)
    ...
    def filter_fortimanager_traffic(
                                    self, packets: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            # JSON-RPC API 호출 패턴 감지
                ...
                # API 호출 메서드 추출 시도
                    method_match = re.search(
                                             r'"method"\s*:\s*"([^"]+)"', payload_str)
                    if method_match:
                        api_method = method_match.group(1)
                        packet["api_method"] = api_method
                        packet["api_method_type"] = self._classify_api_method(
                                                                              api_method)
            ...
            # 인증 정보 감지 (보안 목적)
                    if any(keyword in payload_str.lower()
                           for keyword in ["password", "passwd", "secret", "token"]):
                               packet["contains_credentials"] = True

            # SSL/TLS 정보 (암호화된 트래픽)
                    if packet.get(
                                  "protocol") == "TCP" and not payload_str.isprintable():
                        packet["encrypted_content"] = True
                    packet["payload_size"] = (
                                              len(
                                                                                                payload) if isinstance(payload,
                                                                                                    (bytes,
                                                                             bytearray)) else len(payload_str
                                              )
                    )
    ...
    def _extract_error_info(self, packet: Dict[str, Any], payload: str):
            ...
            error_data = re.search(r'"error"\s*:\s*(\{[^}]+\})', payload)
                if error_data:
                    try:
                        ...
                        packet["error_message"] = error_json.get(
        ...
        try:
            ...
            # 방향별 통계
            inbound = sum(1 for p in fortimanager_packets if p.get(
                          "fortimanager_direction") == "inbound")
            ...
            # 통계 업데이트
                ...
                self.statistics["api_calls_detected"] + = (
                                                           sum(1 for p in fortimanager_packets if p.get(
                                                               "api_call_detected"))
                )
            ...
            logger.error(f"FortiManager 패킷 분석 오류: {e}")
                ...
                def _analyze_response_times(
                                            self, packets: List[Dict[str, Any]]) -> Dict[str, Any]:
                    """
        요청/응답 시간 분석
        ...
        try:
            ...
            # 통계 계산
                ...
                if method_times:
                    method_averages = {
                        m: sum(times) / len(times) for m,
                            times in method_times.items()}
                        slowest_method = max(
                                             method_averages.items(), key=lambda x: x[1])
        ...
        try:
            ...
            # 에러 요약
                        error_summary = [{"code": code, "count": count}
        ...
        try:
            ...
            # 암호화되지 않은 트래픽
            unencrypted = sum(
                              1 for p in packets if not p.get(
            ...
            # 비정상적인 API 호출 패턴
            api_methods = [p.get("api_method")
                                 for p in packets if p.get("api_method")]
        ...
        try:
                ...
                return {
                    "requests_per_second": requests_per_second,
                        "large_payloads_count": len(large_payloads),
                                                    # 최대 5개
                                                "large_payloads": large_payloads[:5],
            ...
            logger.error(f"성능 분석 오류: {e}")
                ...
                def _calculate_risk_level(
                                          self, security_issues: List[Dict[str, Any]]) -> str:
                    """
        위험 수준 계산
    ...
    def _calculate_performance_grade(
                                     self, response_times: Dict[str, Any], requests_per_second: float) -> str:
        """
        성능 등급 계산
