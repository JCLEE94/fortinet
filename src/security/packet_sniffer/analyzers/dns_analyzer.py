...
class DnsAnalyzer(BaseProtocolAnalyzer):
    ...
    def can_analyze(self, packet: PacketInfo) -> bool:
        ...
        if (packet.dst_port == 443 or packet.src_port ==
            443) and packet.protocol == "TCP":
            return True
    ...
    def _get_opcode_name(self, opcode: int) -> str:
        ...
        return opcodes.get(opcode, f"Unknown({opcode})")
            ...
            def _parse_questions(
                                 self, payload: bytes, offset: int, count: int) -> Tuple[List[Dict], int]:
                """DNS 질문 섹션 파싱"""
        questions = []
                ...
                "is_query": not flags.get("qr", False),
                                          "is_response": flags.get("qr", False),
                                                               "is_authoritative": flags.get("aa", False),
                                                                                             "is_truncated": flags.get(
                                                                                                                                                                                                                    "tc",
                                                                                                                                                                                                                        False)
                                                                                             )
                                                                                                                                                                                                  "recursion_desired": flags.get(
                                                                                                                                                                                                                                 "rd",
                                                                                                                                                                                                                                     False)
                                                                                                 )
                                                                                                           "recursion_available": flags.get(
                                                                                                                                            "ra",
                                                                                                                                                                                                               False
                                                                                     )
                                                                                                                                                                        "has_error": flags.get(
