...
logger = logging.getLogger(__name__)
    ...
    def analyze(self, packet_data: bytes,
        ...
        try:
            ...
            # 프로토콜 패턴 기반 자동 감지
                        detected_protocols = self._detect_protocols_by_pattern(
                                                                               payload)
                for proto in detected_protocols:
                    if proto not in analysis["protocols"]:
                        analysis["protocols"].append(proto)
                        proto_analysis = self._analyze_protocol(
                                                                proto, payload, packet_info)
                        if proto_analysis:
                            analysis["application_data"][proto.lower()
                                                                     ] = proto_analysis

            # 세션 정보 업데이트
                            session_info = self._update_session_info(
                                                                     analysis, packet_info)
    ...
    def _extract_payload(self, packet_data: bytes,
                         packet_info: Dict[str, Any]) -> Optional[bytes]:
        """애플리케이션 페이로드 추출"""
        try:
            # IP 헤더 길이 계산
            ...
            # TCP/UDP 헤더 건너뛰기
                ...
                if protocol == "TCP":
                    if len(packet_data) < ip_header_length + 20:
                        return None
                tcp_header_length = (
                                     (packet_data[ip_header_length + 12] >> 4) & 0xF) * 4
    ...
    def _detect_protocols_by_pattern(self, payload: bytes) -> List[str]:
        ...
        detected = []
            ...
            # POP3 감지
                    if payload.startswith(
                                          (b"+OK ", b"-ERR ", b"USER ", b"PASS ", b"QUIT ")):
            ...
            # DNS 감지 (UDP)
                ...
                try:
                    flags = struct.unpack(">H", payload[2:4])[0]
                        if (flags & 0x8000) == 0 or (
                            flags & 0x8000) == 0x8000:  # QR bit
                        detected.append("DNS")
                            except Exception:
                                pass
    ...
    def _analyze_protocol(self, protocol: str, payload: bytes,
    ...
    def _analyze_ssh(self, payload: bytes,
                     packet_info: Dict[str, Any]) -> Dict[str, Any]:
        """SSH 프로토콜 분석"""
        try:
            ...
            logger.error(f"SSH 분석 오류: {e}")
                ...
                def _analyze_http(self, payload: bytes,
                                  packet_info: Dict[str, Any]) -> Dict[str, Any]:
                    """HTTP 프로토콜 분석"""
        try:
            ...
            logger.error(f"HTTP 분석 오류: {e}")
                ...
                def _analyze_ftp(self, payload: bytes,
                                 packet_info: Dict[str, Any]) -> Dict[str, Any]:
                    """FTP 프로토콜 분석"""
        try:
            ...
            # FTP 응답 코드 분석
                    ...
                    analysis.update(
                                    {"type": "response", "code": code, "message": message})
            ...
            logger.error(f"FTP 분석 오류: {e}")
                ...
                def _analyze_smtp(self, payload: bytes,
                                  packet_info: Dict[str, Any]) -> Dict[str, Any]:
                    """SMTP 프로토콜 분석"""
        try:
            ...
            logger.error(f"SMTP 분석 오류: {e}")
                ...
                def _analyze_pop3(self, payload: bytes,
                                  packet_info: Dict[str, Any]) -> Dict[str, Any]:
                    """POP3 프로토콜 분석"""
        try:
            ...
            logger.error(f"POP3 분석 오류: {e}")
                ...
                def _analyze_imap(self, payload: bytes,
                                  packet_info: Dict[str, Any]) -> Dict[str, Any]:
                    """IMAP 프로토콜 분석"""
        try:
            ...
            logger.error(f"IMAP 분석 오류: {e}")
                ...
                def _analyze_mqtt(self, payload: bytes,
                                  packet_info: Dict[str, Any]) -> Dict[str, Any]:
                    """MQTT 프로토콜 분석"""
        try:
            ...
            # 메시지별 상세 분석
                ...
                analysis.update(
                                self._analyze_mqtt_connect(payload[offset + 1:]))
                    elif msg_type == 3:  # PUBLISH
                analysis.update(self._analyze_mqtt_publish(
                                payload[offset + 1:], qos_level))
                    elif msg_type == 8:  # SUBSCRIBE
                analysis.update(
                                self._analyze_mqtt_subscribe(payload[offset + 1:]))
            ...
            logger.error(f"MQTT 분석 오류: {e}")
                ...
                def _analyze_mqtt_connect(
                                          self, payload: bytes) -> Dict[str, Any]:
                    """MQTT CONNECT 메시지 분석"""
        try:
            if len(payload) < 10:
                return {}
            ...
            protocol_name = payload[2: 2 +
     protocol_name_length].decode("utf-8", errors="ignore")
    ...
    def _analyze_mqtt_publish(self, payload: bytes,
                              qos_level: int) -> Dict[str, Any]:
        """MQTT PUBLISH 메시지 분석"""
        try:
            if len(payload) < 2:
                return {}
            ...
            topic_name = payload[2: 2 +
     topic_length].decode("utf-8", errors="ignore")
                offset = 2 + topic_length

            _result = {"topic": topic_name}
    ...
    def _analyze_mqtt_subscribe(self, payload: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            topics = []
                ...
                # 토픽 필터
                topic_filter = payload[offset: offset +
     topic_length].decode("utf-8", errors="ignore")
                    offset += topic_length
    ...
    def _analyze_snmp(self, payload: bytes,
                      packet_info: Dict[str, Any]) -> Dict[str, Any]:
        """SNMP 프로토콜 분석"""
        try:
            ...
            logger.error(f"SNMP 분석 오류: {e}")
                ...
                def _analyze_generic(
                                     self, payload: bytes, packet_info: Dict[str, Any]) -> Dict[str, Any]:
                    """일반적인 애플리케이션 데이터 분석"""
        try:
            analysis = {
                "protocol": "GENERIC",
            ...
            if analysis["is_printable"] and len(payload) < 500:
                try:
                    analysis["text_preview"] = payload.decode(
                                                              "utf-8", errors="ignore")[:200]
                        except Exception:
                            pass
            ...
            logger.error(f"일반 분석 오류: {e}")
                ...
                def _update_session_info(
                                         self, analysis: Dict[str, Any], packet_info: Dict[str, Any]) -> Dict[str, Any]:
                    """세션 정보 업데이트"""
        try:
            session_key = (f"{packet_info.get('src_ip')}:{packet_info.get('src_"
                f"port')}-{packet_info.get('dst_ip')}:{packet_info.get('dst_port')}")
        ...
        issues = []
            ...
            if "SSH" in protocols:
                ssh_data = analysis.get("application_data", {})
                    _version = ssh_data.get("software_version", "")
