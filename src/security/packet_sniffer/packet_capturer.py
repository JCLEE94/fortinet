        ...
        with self.lock:
            if len(self.packets) < self.max_size:
                ...
                self.packets[self.current_index] = packet
                    self.current_index = (
                                          self.current_index + 1) % self.max_size

                    self.total_packets += 1

    def get_packets(self, limit: Optional[int] = None,
                    since: Optional[datetime] = None) -> List[PacketInfo]:
        """패킷 조회"""
        with self.lock:
            ...
            # 시간 필터링
                if since:
                    ...
                    packets = [
                        p for p in packets if p.timestamp >= since_timestamp]
    ...
    def _capture_worker(self, session_id: str, interface: str) -> None:
        ...
        self.logger.info(f"캡처 작업자 시작: {session_id} (인터페이스: {interface})")
            ...
            if use_mock:
                ...
                def _mock_capture_loop(
                                       self, session_id: str, interface: str) -> None:
                    """가짜 패킷 캡처 루프"""
        ...
        self.logger.info(f"Mock 캡처 루프 시작: {session_id}")
            ...
            while self.active_sessions.get(
                                           session_id, False) and packet_count < max_packets:
                try:
                    # 가짜 패킷 생성
                ...
                # 필터 적용
                    if self._should_capture_packet(session_id, packet):
                        ...
                        packet_count += 1
                    self.logger.debug(
                                      f"Mock 패킷 생성됨: {packet_count}/{max_packets}")
                        else:
        ...
        try:
            ...
            success = self._fortigate_client.start_packet_capture(
                                                                  **capture_config)
                if not success:
            ...
            try:
                if self._fortigate_client:
                    ...
                    def _parse_fortigate_packet(
                                                self, packet_data: Dict[str, Any]) -> Optional[PacketInfo]:
                        """FortiGate 패킷 데이터 파싱"""
        try:
            packet = PacketInfo(
                                timestamp=packet_data.get(
                                                          "timestamp", time.time()),
                                                           src_ip=packet_data.get(
                                                                                 "src_ip", ""),
                                                                                     dst_ip=packet_data.get(
                                                                                                        "dst_ip", ""),
            ...
            metrics = {
                "packets_per_second": stats.get("packets_per_second", 0),
                                                "buffer_usage_percent": buffer_stats["buffer_usage"],
                                                    "active_sessions": len(self.active_sessions),
                                                                       "memory_usage": len(
                                                                                                                                                                  self.packet_buffer.packets) * 1024,
                                                                                                                                                                      # 추정
                                                                       )
                                                                                              "error_rate": self.capture_stats["errors"] / max(
                                                                                                                                               self.capture_stats["total_captured"],
                                                                                                                                                   1
                                               )
                                                             }
...
def create_packet_capturer(config: Optional[SnifferConfig] = None) -> PacketCapturer:
    """패킷 캡처러 생성"""
