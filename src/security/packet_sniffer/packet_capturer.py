...
from .base_sniffer import BaseSniffer, SnifferConfig, PacketInfo, MockDataGenerator
...
class PacketCapturer(BaseSniffer):
    ...
    def start_capture_session(self, session_id: str, 
    ...
    def _capture_worker(self, session_id: str, interface: str) -> None:
        ...
        try:
            ...
            use_mock = (
                self.config.offline_mode or 
                self.config.mock_data or 
                not self._fortigate_client or
                interface == 'any'  # 'any' 인터페이스는 mock 모드로 처리
            )
    ...
    def get_latest_packets(self, session_id: Optional[str] = None, 
    ...
    def get_capture_status(self, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if session_id:
            ...
            return {
                ...
                'start_time': self.capture_stats['start_time'].isoformat() if self.capture_stats['start_time'] else None
            }
