        ...
        try:
            ...
            for dataset in data_sets:
                ...
                if data_type == "packets":
                    _result = self.export_packets(data, str(file_path))
                        elif data_type == "analysis":
                    _result = self.export_analysis_results(data, str(file_path))
                        elif data_type == "statistics":
                    _result = self.export_statistics(data, str(file_path))
                        elif data_type == "configuration":
                    _result = self.export_configuration(data, str(file_path))
                        else:
                            # 일반 데이터
                    _result = self._export_general_data(data, str(file_path), name)
                    ...
                    if result["success"]:
                        total_exported += _result.get("exported_count", 1)
            ...
            if file_path.endswith(".gz"):
                with gzip.open(file_path, "rt", encoding="utf-8") as f:
                    data = json.load(f)
                        ...
                        return {"success": True, "data": data, "file_size": path.stat().st_size}
