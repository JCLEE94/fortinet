...
logger = logging.getLogger(__name__)
    ...
    def export_packets(
        ...
        try:
            ...
            logger.error(f"PCAP 내보내기 오류: {e}")
                ...
                def export_filtered_packets(
                                            self, packets: List[Dict[str, Any]], filter_expression: str, output_path: str
                                    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # 필터링된 패킷 내보내기
            _result = self.export_packets(filtered_packets, output_path, include_metadata=True)
        ...
        try:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                ...
                _result = self.export_packets(protocol_packets, str(file_path))
                    result["protocol"] = protocol
                    result["packet_count"] = len(protocol_packets)
                    results[protocol] = _result
                    ...
                    "success": True,
                        "output_directory": str(output_path),
                                                "protocols": list(protocols.keys()),
                                                              "protocol_counts": {p: len(
                                                                                         pkts) for p,
                                                                                                                                                           pkts in protocols.items(
                                                                                                                                                                                   )
                                                                           "results": results,
                                                                               }
            ...
            logger.error(f"프로토콜별 PCAP 내보내기 오류: {e}")
                ...
                def export_time_sliced(
                                       self, packets: List[Dict[str, Any]], output_dir: str, slice_duration: int = 3600
                               ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for slice_key, slice_packets in time_slices.items():
                ...
                _result = self.export_packets(slice_packets, str(file_path))
                    result["time_slice"] = slice_key
                    result["packet_count"] = len(slice_packets)
                    results[slice_key] = _result
    ...
    def _create_tcp_header(self, packet: Dict[str, Any]) -> bytes:
        ...
        flags = 0x18  # PSH + ACK
        _window = packet.get("window_size", 8192)
            checksum = 0
        urgent = 0
    ...
    def _write_metadata_file(self, metadata_path: str, packets: List[Dict[str, Any]], exported_count: int):
        """메타데이터 파일 작성"""
        try:
            metadata = {
                "export_info": {
