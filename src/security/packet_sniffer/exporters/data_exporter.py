    ...
    def export_filtered_data(
        ...
        try:
            ...
            if export_format.lower() == "json":
                _result = self.export_to_json(filtered_packets, session_id, filename)
                    if result["success"]:
                        result["filter_info"] = filter_info
                        elif export_format.lower() == "csv":
                _result = self.export_to_csv(filtered_packets, session_id, filename)
                    if result["success"]:
                        result["filter_info"] = filter_info
                        else:
                            return {
        ...
        filtered = []
            ...
            if "time_range" in filter_criteria:
                time_range = filter_criteria["time_range"]
                    ...
                    if not (time_range.get("start", 0) <= packet_time <= time_range.get("end", float("in"))):
                        include_packet = False
                    continue
