...
class CaptureSession:
    ...
    def add_callback(self, callback: Callable[[PacketInfo], None]) -> None:
        ...
        # 통계 업데이트 스레드 시작
            self._stats_thread = threading.Thread(
                                                  target = (
                                                      self._stats_updater, daemon=True, name=f"stats_{self.session_id[:8]}"
                                                  )
                )
    ...
    def stop(self) -> bool:
        ...
        # 통계 스레드 종료 대기
            if self._stats_thread and self._stats_thread.is_alive():
                ...
                self.logger.info(
                                 f"세션 중지됨: {self.session_id} (상태: {status.value})")
            return True

    def get_packets(
                    self, limit: Optional[int] = None, offset: int = 0) -> List[PacketInfo]:
        """패킷 조회"""
        with self.packet_lock:
    ...
    def add_packet_to_session(self, session_id: str,
                              packet: PacketInfo) -> bool:
        """세션에 패킷 추가"""
        session = self.get_session(session_id)
           if session:
    ...
    def register_session_callback(
                                  self, session_id: str, callback: Callable) -> bool:
        """세션 콜백 등록"""
        session = self.get_session(session_id)
           if session:
        ...
        with self.session_lock:
            total_sessions = len(self.sessions)
               running_sessions = len([s for s in self.sessions.values() if s.info.status == SessionStatus.RUNNING])
                paused_sessions = len(
                                      [s for s in self.sessions.values() if s.info.status == SessionStatus.PAUSED])
                completed_sessions = len(
                                         [s for s in self.sessions.values() if s.info.status == SessionStatus.COMPLETED])
            ...
            with self.session_lock:
                ...
                if session.info.status in [
                    ...
                    if session.info.end_time:
                        age = (
                               current_time -
     session.info.end_time).total_seconds()
