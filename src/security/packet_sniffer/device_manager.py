...
class DeviceManager:
    ...
    def __init__(self, config: Optional[SnifferConfig] = None):
            ...
            try:
                ...
                # 기본 인터페이스 추가
                    ...
                    def _get_interface_details(
                                               self, interface_name: str) -> Optional[NetworkInterface]:
                        """인터페이스 상세 정보 조회"""
        if not HAS_NETIFACES:
            return None
        ...
        try:
            ...
            # 루프백 인터페이스 감지
            is_loopback = (
                           interface_name.startswith("lo") or interface_name.startswith(
                                                     "Loopback") or "127.0.0.1" in ip_addresses
                               )

            return NetworkInterface(
                                    name=interface_name,
                                        display_name=self._get_interface_display_name(
                                                                                      interface_name),
                                                                                          _description=self._get_interface_description(
                                                                                                                                   interface_name),
            ...
            default_interfaces = [
                NetworkInterface(
                                 name="any",
                                     ...
                                     _description="Capture on all available interfaces",
                                         is_up=True,
                                             ),
                                    NetworkInterface(
                                                     name="lo",
                                                         display_name="Loopback",
                                                         _description="Loopback interface",
                                                     ip_addresses=[
                                                         "127.0.0.1"],
                                                             is_up=True,
                                    is_loopback=True,
        ...
        with self.device_lock:
            return [interface.to_dict()
                ...
                def get_interface_details(
                                          self, interface_name: str) -> Optional[Dict[str, Any]]:
                    """특정 인터페이스 상세 정보"""
        with self.device_lock:
                ...
                device = FortiGateDevice(
                                         host=self.config.fortigate_host,
                                             name=status_data.get(
                                                                  "hostname", "FortiGate"),
                                                                      model=status_data.get(
                                                                                        "model", "Unknown"),
                                                                                                                                                                                _version=status_data.get(
                                                                                                                                                                                                         "version",
                                                                                                                                                                                                             "Unknown")
                                                                                        )
                                                                                                                                                                                        serial = status_data.get(
                                                                                                                                                                                                                 "serial",
                                                                                                                                                                                                                     "Unknown")
                                                                                            )
                                                                                                  status="connected",
