...
class SnifferConfig:
    ...
    def __post_init__(self):
        """환경변수에서 설정 로드"""
        self.offline_mode = os.getenv(
                                      "OFFLINE_MODE", "false").lower() == "true"
...
class BaseSniffer(ABC):
            ...
            if callback not in self._callbacks:
                self._callbacks.append(callback)
                   self.logger.debug(f"콜백 추가됨: {callback.__name__}")
                    ...
                    def remove_callback(
                                        self, callback: Callable[[PacketInfo], None]) -> None:
                        """패킷 처리 콜백 제거"""
        with self._lock:
                ...
                self._callbacks.remove(callback)
                   self.logger.debug(f"콜백 제거됨: {callback.__name__}")
                    ...
                    def _notify_callbacks(
                                          self, packet_info: PacketInfo) -> None:
                        """등록된 콜백들에게 패킷 정보 전달"""
            ...
            stats = self.stats.copy()
               if stats["start_time"]:
                    ...
                    if stats["uptime_seconds"] > 0:
                        stats["packets_per_second"] = stats["packets_captured"] / \
    ...
    def get_confidence_score(
                             cls, protocol: str, port: int, payload: bytes) -> float:
        """프로토콜 식별 신뢰도 계산"""
        score = 0.0

        # 포트 기반 점수
        if cls.identify_by_port(port, "TCP") == protocol or cls.identify_by_port(
                                port, "UDP") == protocol:
            score += 0.7
        ...
        app_protocol = ProtocolIdentifier.identify_by_port(dst_port, protocol)
           if app_protocol and app_protocol in payload_samples:
                payload = payload_samples[app_protocol]
                    ...
                    payload = bytes([secrets.randbelow(256)
