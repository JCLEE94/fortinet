...
from .packet_sniffer.session_manager import SessionManager, get_session_manager, create_capture_session
from .packet_sniffer.packet_capturer import PacketCapturer, create_packet_capturer, CaptureFilter
from .packet_sniffer.device_manager import DeviceManager
from .packet_sniffer.base_sniffer import SnifferConfig, PacketInfo, MockDataGenerator
...
class PacketSnifferAPI:
    ...
    def start_capture_session(self, params: Dict[str, Any], 
    ...
    def get_latest_packets(self, session_id: str, count: int = 10, 
        ...
        try:
            ...
            return {
                ...
                'packets': packets,
                'count': len(packets),
                'total_available': self.session_manager.get_session(session_id).get_packet_count() if self.session_manager.get_session(session_id) else 0
            }
    ...
    def filter_packets(self, packets: List[Dict[str, Any]], 
    ...
    def analyze_packet_content(self, packet: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # HTTP 분석
            if self.http_analyzer and packet.get('protocol', '').upper() == 'TCP' and packet.get('dst_port') in [80, 8080]:
                analysis_result['http_analysis'] = self.http_analyzer.analyze_packet(packet)
            
            # TLS 분석
            if self.tls_analyzer and packet.get('protocol', '').upper() == 'TCP' and packet.get('dst_port') in [443, 8443]:
