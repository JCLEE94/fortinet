...
logger = get_logger(__name__, "advanced")
    ...
    def __init__(self, fortigate_client=None,
                 faz_client=None, fmg_client=None):
        """
        ...
        # 설정 생성
            ...
            if fortigate_client:
                ...
                config.fortigate_token = getattr(
                                                 fortigate_client, "api_token", "")
        ...
        try:
            return self.device_manager.get_available_devices()
                ...
                def get_device_interfaces(
                                          self, device_name: str) -> List[Dict[str, Any]]:
                    """장치의 인터페이스 목록 조회"""
        try:
        ...
        try:
            ...
            session_id = create_capture_session(
                                                name=params.get("name", ""),
                                                                _description=params.get(
                                                                                        "description", ""),
                                                                                                                                                                                max_duration=params.get(
                                                                                                                                                                                                        "duration",
                                                                                                                                                                                                            3600)
                                                                                        )
                                                                                                                                                                                                        max_packets = params.get(
                                                                                                                                                                                                                                 "max_packets",
                                                                                                                                                                                                                                     10000)
                                                                                                    )
                                                                                                        )
                            ...
                            "total_packets": total_packets,
                                "total_bytes": total_bytes,
                                    "protocol_distribution": protocol_stats,
                                                                                "top_ports": dict(
                                                                                                  sorted(port_stats.items(),
                                                                                                      key=lambda x: x[1],
                                                       reverse=True)[:10])
                                        )
                                                  "top_connections": dict(
                                                                          sorted(ip_pairs.items(),
                                                                                 key=lambda x: x[1],
                                                                                                                               reverse=True)[:10])
                                                      )
                                              },
