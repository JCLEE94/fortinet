...
class SecurityScanner:
    ...
    def auto_fix_vulnerabilities(self, scan_result: Dict = None) -> Dict:
        ...
        for vuln in scan_result.get("vulnerabilities", []):
            if vuln.get("auto_fixable", False):
                ...
                fix_results["manual_intervention_required"].append(vuln)
    ...
    def _configure_fail2ban(self) -> Dict:
        ...
        try:
            # Fail2ban 설치 확인
            result = subprocess.run(["which", "fail2ban-server"], capture_output=True)
...
if __name__ == "__main__":
    ...
    print(f"보안 스캔 완료: {json.dumps(result, indent=2, ensure_ascii=False)}")
