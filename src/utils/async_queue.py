    ...
    def __init__(self, max_size: int = 10000, worker_count: int = 10):
        ...
        for i in range(self.worker_count):
            worker = asyncio.create_task(self._worker(f"worker-{i}"))
                ...
                logger.info(
                            f"AsyncQueue started with {self.worker_count} workers")
    ...
    async def _worker(self, worker_name: str):
        ...
        start_time = time.time()
            ...
            # 처리 시간 기록
                ...
                logger.debug(
                    ...
                    logger.error(
                                 f"Error processing item in {worker_name}: {e}")
            ...
            # 재시도 로직
                ...
                await asyncio.sleep(2**item.retry_count)
                    ...
                    logger.error(
                                 f"Item failed after {item.max_retries} retries")
        ...
        if self._processing_times:
            avg_processing_time = sum(
                                      self._processing_times) / len(self._processing_times)
                            ...
                            "pending": self._stats["pending"],
                                "worker_count": self.worker_count,
                                    "avg_processing_time": avg_processing_time,
                                                                "queue_by_priority": {priority.name: len(
                                                                                                         queue) for priority,
                                                                                                             queue in self.queues.items()}
                                )
                                                                     }
    ...
    async def add_item(self, item: Any):
        ...
        async with self._lock:
            ...
            # 배치 크기에 도달하거나 시간이 경과한 경우 플러시
                should_flush = len(self.batch) >= self.batch_size or time.time(
        ...
        batch_to_process = self.batch[:]
            ...
            try:
                if asyncio.iscoroutinefunction(self._processor_callback):
                    ...
                    logger.debug(
                                 f"Processed batch of {len(batch_to_process)} items")
...
# 편의 함수들
    ...
    async def create_queue(name: str, max_size: int=10000,
                           worker_count: int=10) -> AsyncQueue:
    """큐 생성 및 시작"""
    queue = queue_manager.get_queue(
                                    name, max_size = max_size, worker_count = worker_count)
        await queue.start()
        return queue
