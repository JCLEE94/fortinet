...
# from utils.api_utils import get_data_source, is_test_mode,
    # get_api_manager  # 순환 임포트 방지
...
def get_env_bool(key: str, default: bool = False) -> bool:
    ...
    if default is None:
        ...
        if value:
            return [item.strip()
                               for item in value.split(separator) if item.strip()]
            return default
...
# 컨텍스트 매니저
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        duration = time.time() - self.start_time
           if exc_type:
                self.logger.error(
                                  f"실패: {self.operation_name} ({duration:.2f}초)")
                else:
                    self.logger.info(
                                     f"완료: {self.operation_name} ({duration:.2f}초)")
