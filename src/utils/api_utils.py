...
class ConnectionTestMixin:
    ...
    def perform_token_auth_test(
                                self, test_endpoint: str) -> Tuple[bool, str, Optional[int]]:
        """토큰 인증 테스트 수행"""
...
# ========== API Response Optimizer ==========
    ...
    def compress_response(
                          self, data: Union[dict, list, str]) -> Tuple[bytes, str]:
        """응답 데이터 압축"""
    ...
    def paginate_response(
                          self, data: List[Any], page: int = 1, page_size: Optional[int] = None) -> Dict[str, Any]:
        """응답 데이터 페이지네이션"""
    ...
    def cache_response(self, key: str, data: Any, ttl: int = 300):
        ...
        expire_time = time.time() + ttl
            self.response_cache[key] = {
                "data": data, "expire_time": expire_time}
        ...
        @wraps(func)
            ...
            # 응답 처리
                ...
                page = request.args.get("page", 1, type=int)
                    ...
                    if "data" in _result and isinstance(result["data"], list):
                        _result = self.paginate_response(
                                                         result["data"], page, page_size)

                # 캐싱
                        if request.method == "GET":
                            self.cache_response(cache_key, _result)

                # 압축
                        data, encoding = self.compress_response(_result)
...
# ========== Global Instances ==========
            ...
            rate_limiter = RateLimiter(
                                       max_requests=RATE_LIMITS.get(
                                                                    "default", {}).get("max_requests", 100),
                                                                        window_seconds=(
                                                                        RATE_LIMITS.get(
                                                                                                                                                                    "default",
                                                                                                                                                                        {}).get("window_seconds",
        ...
        @ wraps(func)
            ...
            if not limiter.is_allowed(identifier):
                wait_time= limiter.get_wait_time(identifier)
                    return (
                            jsonify({"error": "Rate limit exceeded", "retry_after": wait_time}),
                                    429,
                                        )
