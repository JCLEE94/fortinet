...
class BatchProcessor:
    ...
    def process_batch(
                      self, items: List[BatchItem], progress_callback: Optional[Callable] = None) -> List[BatchResult]:
        """
        배치 작업 처리
    ...
    def _process_sequential(
                            self, items: List[BatchItem], progress_callback: Optional[Callable]) -> List[BatchResult]:
        """순차적 처리"""
        results = []
           total = len(items)

            for idx, item in enumerate(items):
                _result = self._execute_item(item)
                results.append(_result)

                if progress_callback:
                    progress_callback(idx + 1, total, _result)
    ...
    def _process_parallel_thread(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            # 결과 수집
                ...
                if progress_callback:
                    progress_callback(completed, total, _result)
        ...
        # 배치 단위로 처리
            ...
            # 결과 처리
                ...
                results.append(_result)
                   ...
                   if progress_callback:
                        progress_callback(i + idx + 1, total, _result)
    ...
    def _execute_item(self, item: BatchItem) -> BatchResult:
        ...
        start_time = time.time()
           ...
           try:
            ...
            _result = item.operation(item.data, *args, **kwargs)
        ...
        start_time = time.time()
           ...
           try:
            ...
            if asyncio.iscoroutinefunction(item.operation):
                _result = await item.operation(item.data, *args, **kwargs)
                   else:
                        # 동기 함수를 비동기로 실행
                loop = asyncio.get_event_loop()
                   _result = await loop.run_in_executor(None, item.operation, item.data, *args)
...
def _execute_item_wrapper(
                          operation: Callable, data: Any, args: Optional[Tuple], kwargs: Optional[Dict]
    ...
    start_time = time.time()
       ...
       try:
            args = args or ()
            kwargs = kwargs or {}
        _result = operation(data, *args, **kwargs)
           execution_time = time.time() - start_time
        ...
        for result in results:
            if result.success:
                device_map[result.item_id] = result.result
                        ...
                        logger.error(
                                     f"Failed to get device {result.item_id}: {result.error}")

                    return device_map

    def batch_update_policies(
                              self, policy_updates: List[Dict[str, Any]]) -> List[BatchResult]:
        """
        여러 정책 일괄 업데이트
