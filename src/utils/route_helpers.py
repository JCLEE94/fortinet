...
def standard_api_response(success: bool = True, data: Any = None, 
                         message: str = "", status_code: int = 200) -> tuple:
    """표준화된 API 응답 생성"""
    response = {
...
def api_route(methods: list = ['GET'], require_auth: bool = False, 
              rate_limits: Dict[str, int] = None) -> Callable:
    """통합 API route 데코레이터"""
    def decorator(f: Callable) -> Callable:
        # Rate limiting
        if rate_limits:
