    ...
    "X-XSS-Protection": "1; mode=block",
        ...
        "Permissions-Policy": "geolocation=(self), microphone=(), camera=(), payment=()",
                                        ...
                                        "Content-Security-Policy": (
                                                                    "default-src 'sel'; "
        "script-src 'sel' 'unsafe-inline' 'unsafe-eval' https://cdnjs.cloudflare.com https://cdn.jsdelivr.net; "
        "style-src 'sel' 'unsafe-inline' https://cdnjs.cloudflare.com https://fonts.googleapis.com; "
        "font-src 'sel' https://cdnjs.cloudflare.com https://fonts.gstatic.com data:; "
        "img-src 'sel' data: https:; "
        "connect-src 'sel' ws: wss: https:; "
        "frame-ancestors 'sel'; "
        "base-uri 'sel'; "
        "form-action 'self'"
        ),
...
def generate_csrf_token():
    ...
    def validate_hostname(hostname):
        ...
        hostname_pattern = re.compile(
                                      r"^ [a - zA - Z0 - 9](
                                                            [a-zA-Z0-9\- ]{0,
                                                                61}[a-zA-Z0-9])?" r"(\.[a-zA-Z0-9]([a-zA-Z0-9\- ]{0,
                                                                              61}[a - zA - Z0 - 9])?) *$
                                              )
                                      )
...
class RateLimiter:
    ...
    def is_allowed(self, ip, endpoint, max_requests=None, _window=None):
        """요청 허용 여부 확인"""
        if max_requests is None:
            max_requests = RATE_LIMITS["MAX_REQUESTS"]
                if _window is None:
                    _window = RATE_LIMITS["WINDOW_SECONDS"]
                now = datetime.now()
        ...
        # 시간 윈도우 밖의 요청 제거
                cutoff_time = now - timedelta(seconds=_window)
...
# 전역 속도 제한기 인스턴스
    ...
    def rate_limit(max_requests=None, _window=None):
        """속도 제한 데코레이터"""
    if max_requests is None:
        max_requests = RATE_LIMITS["MAX_REQUESTS"]
            if _window is None:
            ...
            if not rate_limiter.is_allowed(ip, endpoint, max_requests, _window):
                return (
                        jsonify(
                                {
