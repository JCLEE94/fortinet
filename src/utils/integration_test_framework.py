...
class TestResult:
                       ...
                       _result = func(*args, **kwargs)
                        ...
                        self.results.append(
                                            TestResult(
                                                ...
                                                passed=True,
                                                    duration=duration,
                                                    details=_result if isinstance(
                                                                              _result, dict) else None,
                                                                                  )
                            )
                    self.passed_count += 1
    ...
    def assert_eq(self, actual: Any, expected: Any, message: str = ""):
        ...
        if isinstance(result, dict) and result.get("success") is not False:
            raise AssertionError(
                                 f"Expected failure but got success: {message}")
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        # 결과 요약
            ...
            if self.test_count > 0:
                print(
                      f"Success rate: {(self.passed_count/self.test_count*100):.1f}%")
                else:
