    ...
    def get_logger(
        ...
        for logger in self._loggers.values():
            ...
            for handler in logger.logger.handlers:
                try:
                    # Check if handler's stream is still open before flushing
                    if hasattr(
                               handler, 'stream') and not handler.stream.closed:
                        handler.flush()
                           elif not hasattr(handler, 'stream'):
                            # For handlers without stream attribute (like
                            # SysLogHandler)
                        handler.flush()
                           except (ValueError, AttributeError):
                                # Skip handlers that are already closed or
...
class UnifiedLogger:
    ...
    def _setup_strategy(self, strategy: str, log_dir: str = None):
        ...
        if strategy.lower() == "advanced":
            ...
            def log_with_context(self, level: int, msg: str,
    ...
    # Specialized logging methods for API clients
        ...
        if error or (status_code >= 400):
            self.logger.error(
                              f"API Response Error: {status_code}",
                                  extra=extra)
               else:
                    self.logger.info(
                                     f"API Response: {status_code}", extra=extra)
...
def get_logger(name: str, strategy: str = "basic", log_dir: str = None, log_level: str = None) -> UnifiedLogger:
    """Get a logger instance (compatible with existing code)"""
       return LoggerRegistry().get_logger(name, strategy, log_dir, log_level)
        ...
        def get_advanced_logger(name: str, log_dir: str = None,
                                log_level: str = None) -> UnifiedLogger:
