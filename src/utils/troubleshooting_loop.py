...
from src.config.constants import (
    DEFAULT_PORTS, TIMEOUTS, CHECK_INTERVALS, 
    DEFAULT_PATHS, SERVICE_URLS
)
...
class TroubleshootingLoop:
    ...
    def start(self):
        ...
        # 시작 로깅
        self.logger.info(f"FortiGate 트러블슈팅 시스템 시작: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}, "
            ...
            for filename in os.listdir(log_dir):
                if filename.endswith('.log'):
                    ...
                    if os.path.getsize(filepath) > 10 * 1024 * 1024:  # 10MB 이상
                        ...
                        subprocess.run(['gzip', '-c', filepath], 
