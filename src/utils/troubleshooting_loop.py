...
class TroubleshootingLoop:
        ...
        # 시작 로깅
           self.logger.info(
                            f"FortiGate 트러블슈팅 시스템 시작: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}, "
                             "모니터링 대상: 웹 애플리케이션, Mock 서버, 시스템 리소스, 체크 간격: 30초"
            )
    ...
    def _diagnose_system(self) -> List[Dict[str, Any]]:
        ...
        # Mock 서버 상태 확인
            ...
            if response.status_code != 200:
                issues.append({"type": "mock_server_down",
                              "status_code": response.status_code})
                   except Exception as e:
                        issues.append(
                                      {"type": "mock_server_down", "error": str(e)})
        ...
        for issue in issues:
            ...
            if issue_type in self.recovery_strategies:
                self.logger.info(f"{issue_type} 복구 시도")
                   try:
                        _result = self.recovery_strategies[issue_type](issue)
                        if _result:
                    ...
                    # 복구 오류 로깅
                        ...
                        def _recover_web_app(
                                             self, issue: Dict[str, Any]) -> bool:
                            """웹 애플리케이션 복구"""
        try:
            # 프로세스 재시작
            ...
            # 상태 확인
                response = requests.get(
                                        self.monitors["web_app"]["url"], timeout=5)
    ...
    def _recover_mock_server(self, issue: Dict[str, Any]) -> bool:
        ...
        try:
            # 프로세스 재시작
            subprocess.run(["supervisorctl", "restart",
                           "mock_server"], check=True)
               time.sleep(10)  # 시작 대기 (짧은 대기 시간은 하드코딩 유지)
            ...
            current_time = time.time()
               ...
               for filename in os.listdir(log_dir):
                    ...
                    if os.path.isfile(filepath):
                        # 7일 이상 된 파일 삭제
                    if current_time - \
                        os.path.getmtime(filepath) > 7 * 24 * 3600:
                        os.remove(filepath)
                           self.logger.info(f"오래된 로그 파일 삭제: {filename}")

            # Docker 정리
                            subprocess.run(
                                           ["docker", "system", "prune", "-"], check=True)

            # 임시 파일 정리
                subprocess.run(["find", "/tmp", "-type", "",
                               "-atime", "+7", "-delete"], check=True)
    ...
    def _free_memory(self, issue: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # 사용하지 않는 Docker 컨테이너 정리
                subprocess.run(
                               ["docker", "container", "prune", "-"], check=True)
