...
from src.config.constants import CHECK_INTERVALS, DEFAULT_PATHS, DEFAULT_PORTS, SERVICE_URLS, TIMEOUTS
...
class TroubleshootingLoop:
    ...
    def _diagnose_system(self) -> List[Dict[str, Any]]:
        ...
        # 웹 애플리케이션 상태 확인
        try:
            response = requests.get(self.monitors["web_app"]["url"], timeout=self.monitors["web_app"]["timeout"])
        ...
        try:
            response = requests.get(
                self.monitors["mock_server"]["url"], timeout=self.monitors["mock_server"]["timeout"]
            )
            if response.status_code != 200:
            ...
            for filename in os.listdir(log_dir):
                if filename.endswith(".log"):
                    ...
                    if os.path.getsize(filepath) > 10 * 1024 * 1024:  # 10MB 이상
                        ...
                        subprocess.run(
                            ["gzip", "-c", filepath], stdout=open(os.path.join(log_dir, "archive", archive_name), "wb")
                        )
