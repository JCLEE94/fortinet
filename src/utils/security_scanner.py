...
class SecurityScanner:
           ...
           for root, dirs, files in os.walk(directory):
            ...
            dirs[:] = [d for d in dirs if not d.startswith("__")]
               ...
               for file in files:
                    ...
                    # 제외 패턴 확인
                       if any(re.search(pattern, file_path)
            ...
            # 각 취약점 카테고리별로 검사
                    for category, config in self.vulnerability_patterns.items():
                       ...
                       for match in matches:
                            ...
                            _line_content = lines[line_num -
     1].strip() if line_num <= len(lines) else ""

                        # 코드 스니펫 생성 (전후 2줄 포함)
                            snippet_lines = []
                            for i in range(max(0, line_num - 2),
    ...
    def analyze_dependencies(
                             self, requirements_file: str) -> List[SecurityVulnerability]:
        """의존성 보안 분석"""
        vulnerabilities = []
                                   ...
                                   _version = line.split("==")[1].strip()
                                    ...
                                    if package_name in vulnerable_packages:
                                        ...
                                        vulnerability = SecurityVulnerability(
                                                ...
                                                severity="medium",
                                                _description=vuln_info["description"],
                                                code_snippet=line,
