    ...
    def get_routing_table(self, device_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            if self.api_client:
                ...
                response = self.api_client.get(
                    device_name=device_name,
                    url="/api/v2/monitor/router/ipv4"
                )
        ...
        try:
            if self.api_client:
                ...
                arp_response = self.api_client.get(
                    device_name=device_name,
                    url="/api/v2/monitor/system/arp"
                )
                
                dhcp_response = self.api_client.get(
                    device_name=device_name,
                    url="/api/v2/monitor/system/dhcp"
                )
    ...
    def search_devices(self, keyword: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for device in fortigate_devices:
                # 이름, 호스트명, IP 등으로 검색
                if (keyword in device.get('name', '').lower() or
                    keyword in device.get('hostname', '').lower() or
                    keyword in device.get('ip', '').lower() or
                    keyword in device.get('platform', '').lower()):
            ...
            for fg_device in fortigate_devices:
                ...
                for device in connected_devices:
                    ...
                    if (keyword in device.get('hostname', '').lower() or
                        keyword in device.get('ip', '').lower() or
                        keyword in device.get('mac', '').lower() or
                        keyword in device.get('type', '').lower()):
                        # FortiGate 정보 추가
...
class DeviceManagerAPI:
    ...
    def _calculate_device_statistics(self, fortigate_devices: List[Dict[str, Any]], 
                                   connected_devices: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        장치 통계 계산
