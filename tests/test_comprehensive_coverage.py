...
class TestConfigurationCoverage(unittest.TestCase):
    ...
    def test_settings_import(self):
        """설정 모듈 임포트 테스트"""
        from src.config.settings import get_settings, DEFAULT_CONFIG
    ...
    def test_config_migration(self):
        """설정 마이그레이션 테스트"""
        from src.config.config_migration import migrate_config, backup_config
...
class TestSecurityCoverage(unittest.TestCase):
    ...
    def test_security_utilities(self):
        """보안 유틸리티 테스트"""
        from src.utils.security import add_security_headers, SECURITY_HEADERS
