...
def test_docker_registry_connection():
    ...
    try:
        ...
        test_framework.assert_ok(
            False,
            f"Registry connection failed: {str(e)}"
        )
...
def test_docker_image_in_registry():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            test_framework.assert_ok(
                len(tags) > 0,
                f"At least one image tag should exist in registry"
            )
        ...
        test_framework.assert_ok(
            False,
            f"Failed to check registry images: {str(e)}"
        )
...
def test_chartmuseum_connection():
    ...
    try:
        ...
        response = requests.get(
            f"{config.chartmuseum_url}/health",
            timeout=10
        )
        ...
        test_framework.assert_ok(
            False,
            f"ChartMuseum connection failed: {str(e)}"
        )
...
def test_helm_chart_in_museum():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            test_framework.assert_ok(
                len(chart_versions) > 0,
                f"At least one chart version should exist"
            )
        ...
        test_framework.assert_ok(
            False,
            f"Failed to check chart availability: {str(e)}"
        )
...
def test_kubernetes_deployment():
    ...
    try:
        ...
        test_framework.assert_ok(
            False,
            f"Failed to parse kubectl output: {cmd_result['stdout']}"
        )
...
def test_kubernetes_service():
    ...
    try:
        ...
        test_framework.assert_ok(
            len(services) > 0,
            f"At least one service should exist"
        )
        ...
        for service in services:
            ...
            service_info = {
                'name': service_name,
                'type': service_type,
                'ports': [{'port': p['port'], 'targetPort': p.get('targetPort'), 'nodePort': p.get('nodePort')} for p in ports]
            }
        ...
        test_framework.assert_ok(
            False,
            f"Failed to parse service kubectl output: {cmd_result['stdout']}"
        )
...
def test_argocd_application():
    ...
    try:
        ...
        test_framework.assert_ok(
            False,
            f"Failed to parse ArgoCD output: {cmd_result['stdout']}"
        )
...
def test_complete_pipeline():
    ...
    # 1. 레지스트리 접근성 확인
    try:
        response = requests.get(f"https://{config.registry_url}/v2/", auth=config.registry_auth, timeout=5, verify=False)
