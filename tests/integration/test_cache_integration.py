...
def test_redis_failure_fallback():
    ...
    test_framework.assert_ok(failure_set, "Cache set should succeed even with Redis failure (memory fallback)")
...
def test_cache_consistency():
    ...
    consistency_tests = [
        ...
        {"key": "consistency_test_4", "value": {"data": {"nested": True}, "type": "object"}}
    ]
...
def test_concurrent_cache_access():
    ...
    # 검증
    test_framework.assert_eq(len(shared_results), total_operations,
                           f"All {total_operations} operations should complete")
    
    test_framework.assert_ok(len(successful_operations) > total_operations * 0.9,
                           "At least 90% of operations should succeed")
    
    test_framework.assert_ok(len(consistent_operations) > total_operations * 0.9,
