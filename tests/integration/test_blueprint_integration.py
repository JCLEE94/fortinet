...
def test_blueprint_registration():
    ...
    with test_framework.test_app() as (app, client):
        # 예상되는 Blueprint 목록
        expected_blueprints = {
            'main', 'api', 'fortimanager', 'itsm', 'itsm_api', 
            'itsm_automation', 'logs', 'performance'
        }
        ...
        test_framework.assert_eq(len(missing_blueprints), 0, 
                               f"Missing blueprints: {missing_blueprints}")
        ...
        for blueprint_name in expected_blueprints:
            test_framework.assert_ok(blueprint_name in blueprint_routes,
                                   f"Blueprint {blueprint_name} should have routes")
            
            routes_count = len(blueprint_routes[blueprint_name])
            test_framework.assert_ok(routes_count > 0,
...
def test_route_conflicts():
    ...
    with test_framework.test_app() as (app, client):
        ...
        test_framework.assert_eq(len(critical_conflicts), 0,
    ...
    with test_framework.test_app() as (app, client):
        ...
        for endpoint, params in critical_endpoints:
            try:
                ...
                # URL이 유효한 형식인지 검증
                test_framework.assert_ok(generated_url.startswith('/'),
        ...
        test_framework.assert_eq(len(failed_urls), 0,
                               f"Failed URL generations: {failed_urls}")
...
def test_error_handler_precedence():
    ...
    with test_framework.test_app() as (app, client):
        ...
        # 404는 처리되어야 함
        test_framework.assert_eq(response.status_code, 404,
        ...
        test_framework.assert_ok(len(response.get_data()) > 0,
        ...
        test_framework.assert_eq(response.status_code, 405,
                               "Wrong method should return 405")
        
        return {
            'error_tests': error_tests,
            'error_handling_working': all(test['status_code'] in [404, 405, 500] 
    ...
    with test_framework.test_app() as (app, client):
        ...
        for url, test_name in pages_to_test:
            try:
                ...
                # 기본적인 HTML 구조 확인
                test_framework.assert_ok(response.status_code in [200, 302],
        ...
        test_framework.assert_eq(len(failed_templates), 0,
                               f"Template rendering failures: {failed_templates}")
    ...
    with test_framework.test_app({'WTF_CSRF_ENABLED': True}) as (app, client):
        ...
        for endpoint in get_endpoints:
            try:
                ...
                test_framework.assert_ok(response.status_code != 400,
