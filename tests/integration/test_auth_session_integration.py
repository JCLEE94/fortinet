...
def test_api_key_authentication():
    ...
    with test_framework.test_app() as (app, client):
        ...
        test_framework.assert_ok(
            response.status_code in [401, 403, 200],  # 200은 공개 API의 경우
            "Missing API key should be handled"
        )
...
def test_api_key_rate_limiting():
    ...
    with test_framework.test_app() as (app, client):
        ...
        with patch('src.core.auth_manager.AuthManager.check_rate_limit', 
    ...
    with test_framework.test_app() as (app, client):
        ...
        with patch('src.core.auth_manager.AuthManager.verify_credentials') as mock_verify:
            ...
            if response.status_code == 200:
                ...
                test_framework.assert_ok(
                    data.get("success") or "session" in data,
        ...
        test_framework.assert_ok(
            response.status_code in [200, 401],  # 401은 세션이 없는 경우
...
def test_session_expiry_and_refresh():
    ...
    with test_framework.test_app() as (app, client):
        ...
        with patch.object(auth_manager, 'refresh_session') as mock_refresh:
            ...
            test_framework.assert_ok(
                refresh_result.get("success"),
                "Session refresh should succeed"
            )
...
def test_redis_session_management():
    ...
    with test_framework.test_app() as (app, client):
        ...
        if cache.redis_enabled:
            ...
            test_framework.assert_ok(
                ttl > 0 or not cache.redis_enabled,
...
def test_concurrent_session_handling():
    ...
    with test_framework.test_app() as (app, client):
        ...
        # 결과 검증
        test_framework.assert_eq(
            session_results["created"],
            10,
            "All sessions should be created"
        )
        test_framework.assert_eq(
            session_results["validated"],
            10,
