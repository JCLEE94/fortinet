...
def test_docker_image_build():
    ...
    try:
        ...
        test_framework.assert_ok(
            False,
            f"Unexpected error during Docker build: {str(e)}"
        )
...
def test_docker_image_layers():
    ...
    try:
        ...
        test_framework.assert_ok(
            len(images) > 0,
            f"Test image {docker_manager.test_image_name} should exist"
        )
        ...
        test_framework.assert_ok(
            False,
            f"Failed to analyze Docker image: {str(e)}"
        )
...
def test_docker_container_startup():
    ...
    try:
        ...
        test_framework.assert_ok(
            is_ready,
            "Container should start and become ready within 60 seconds"
        )
...
def test_docker_container_health():
    ...
    try:
        ...
        test_framework.assert_ok(
            False,
            f"Container health check failed: {str(e)}"
        )
...
def test_docker_container_logs():
    ...
    try:
        ...
        test_framework.assert_ok(
            len(containers) > 0,
            f"Test container should be running for log analysis"
        )
...
def test_docker_cleanup():
    ...
    try:
        # 테스트 컨테이너 정리
        containers = docker_manager.client.containers.list(all=True, filters={'name': docker_manager.test_container_name})
...
def test_production_image_from_registry():
    ...
    try:
        ...
        try:
            ...
            test_framework.assert_ok(
                image is not None,
                f"Should be able to pull image from registry"
            )
