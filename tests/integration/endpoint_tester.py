    ...
    def test_endpoint(self, method, endpoint, data=None, expected_status=200):
        ...
        try:
            ...
            if response.status_code == expected_status:
                ...
                logger.warning(f"⚠️  {method} {endpoint} - {response.status_code} (expected {expected_status}) ({response_time}ms)")
    ...
    def generate_report(self):
        ...
        if successful:
            ...
            for result in successful:
                content_type = result['content_type'][:30] + "..." if result['content_type'] and len(result['content_type']) > 30 else result['content_type']
                print(f"   {result['method']} {result['endpoint']} - {result['status_code']} ({result['response_time_ms']}ms) [{content_type}]")
