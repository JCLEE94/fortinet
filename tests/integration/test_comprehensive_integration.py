...
def test_packet_path_analysis_integration():
    ...
    with test_framework.test_app() as (app, client):
        # API 엔드포인트 테스트
        test_data = {"src_ip": "192.168.1.100", "dst_ip": "10.0.0.50", "dst_port": 443, "protocol": "tcp"}

        response = client.post(
            "/api/fortimanager/analyze-packet-path", json=test_data, headers={"Content-Type": "application/json"}
        )
...
def test_itsm_policy_automation_workflow():
    ...
    with test_framework.test_app() as (app, client):
        ...
        ticket_data = {
            ...
            "details": {"action": "allow", "source": "192.168.1.0/24", "destination": "10.0.0.0/8", "service": "https"},
        }

        with patch.object(automation_service, "create_policy_from_ticket") as mock_create:
            mock_create.return_value = {"success": True, "policy_id": "POL-12345", "status": "pending_approval"}
        ...
        with patch.object(automation_service, "approve_policy") as mock_approve:
            mock_approve.return_value = {"success": True, "status": "approved", "approved_by": "admin@example.com"}
...
def test_itsm_external_system_connectors():
    ...
    with test_framework.test_app() as (app, client):
        ...
        with patch.object(connector, "process_servicenow_webhook") as mock_process:
            mock_process.return_value = {"success": True, "action": "create_ticket", "local_id": "ITSM-001"}
...
def test_configuration_storage_integration():
    ...
    with test_framework.test_app() as (app, client):
        ...
        # 3. 런타임 설정 업데이트
        response = client.post("/api/settings", json={"api_timeout": 90}, headers={"Content-Type": "application/json"})
...
def test_realtime_monitoring_integration():
    ...
    with test_framework.test_app() as (app, client):
        ...
        with patch.object(monitor, "collect_metrics") as mock_collect:
            ...
            test_framework.assert_ok(metrics.get("disk_usage", 0) < 90, "Disk usage should be below threshold")
...
def test_api_performance_monitoring():
    ...
    with test_framework.test_app() as (app, client):
        ...
        for endpoint in endpoints:
            ...
            test_framework.assert_ok(response.status_code in [200, 404], f"{endpoint} should return valid status")
...
def test_cascade_failure_handling():
    ...
    with test_framework.test_app() as (app, client):
        ...
        with patch("src.core.connection_pool.get_connection") as mock_conn:
            ...
            test_framework.assert_ok(response.status_code in [200, 503], "Should handle DB failure gracefully")
        ...
        with patch("requests.Session.get") as mock_get:
            ...
            if response.status_code == 200:
                data = response.get_json()
                test_framework.assert_ok(data.get("cached") or data.get("mock"), "Should indicate cached or mock data")
...
def test_graceful_degradation_modes():
    ...
    with test_framework.test_app({"OFFLINE_MODE": "true"}) as (app, client):
        ...
        # 2. 쓰기 작업 차단
        response = client.post(
            "/api/fortigate/policies", json={"name": "new_policy"}, headers={"Content-Type": "application/json"}
        )

        # 오프라인 모드에서는 쓰기 작업이 차단되거나 큐에 저장됨
        test_framework.assert_ok(response.status_code in [202, 503], "Write operations should be queued or blocked")
