...
class BugFixManager:
    ...
    def fix_argocd_sync_issues(self) -> Dict:
        ...
        code, stdout, stderr = self.run_command("kubectl delete ingress fortinet -n fortinet --ignore-not-found=true")
        if code == 0:
            fixes_applied.append("Deleted invalid Ingress resource")
        
        # 2. 문제 있는 PVC 삭제
        logger.info("문제 있는 PVC 정리...")
        code, stdout, stderr = self.run_command("kubectl delete pvc fortinet-data -n fortinet --ignore-not-found=true --force --grace-period=0")
        ...
        if not fixes_applied:
            ...
            # 기존 애플리케이션 삭제
            self.run_command("kubectl delete application fortinet -n argocd --ignore-not-found=true")
            time.sleep(5)
            
            # 새 애플리케이션 생성
            if os.path.exists("argocd-fortinet-gitops.yaml"):
                code, stdout, stderr = self.run_command("kubectl apply -f argocd-fortinet-gitops.yaml")
                if code == 0:
...
{{- end }}"""
    ...
    def fix_service_port_conflicts(self) -> Dict:
        ...
        code, stdout, stderr = self.run_command("kubectl get svc --all-namespaces | grep NodePort")
    ...
    def fix_docker_issues(self) -> Dict:
        ...
        for compose_file in compose_files:
            if compose_file.exists():
                try:
                    # YAML 구문 검증
                    code, stdout, stderr = self.run_command(f"docker-compose -f {compose_file} config --quiet")
                    if code != 0:
                        logger.warning(f"Docker Compose 파일 {compose_file}에 문제가 있을 수 있습니다: {stderr}")
                except Exception as e:
