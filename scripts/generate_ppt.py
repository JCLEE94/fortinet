...
def create_solution_overview_slide(prs):
    ...
    # 하단 특징들
    features = [("정책 관리", 1.5), ("모니터링", 3.5), ("자동화", 5.5), ("통합 관리", 7.5)]
...
def create_key_features_slide(prs):
    ...
    for title_text, content, left, top in features:
        ...
        shape = slide.shapes.add_shape(
            MSO_SHAPE.ROUNDED_RECTANGLE, Inches(left), Inches(top), width, height
        )
        shape.fill.solid()
...
def main():
    ...
    output_file = os.path.join(
        output_dir, "FortiGate_Nextrade_Enterprise_Evaluation.pptx"
    )
