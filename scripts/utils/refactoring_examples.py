...
def new_way_networks():
    from src.config.network import DEFAULT_GATEWAYS, TEST_IPS, DNS_SERVERS
...
class NewFortiGateClient:
    def __init__(self, host, username=None, password=None, api_key=None, port=None):
        from src.config.ports import get_service_port
        from src.config.services import get_api_endpoint, AUTH_ENDPOINTS
...
class OldMockData:
    def get_network_interfaces(self):
        return {
            'port1': {
                'ip': '192.168.1.1/24',  # 하드코딩
                'alias': 'internal'
            },
            'port2': {
                'ip': '172.16.1.1/24',  # 하드코딩
                'alias': 'dmz'
            }
        }
