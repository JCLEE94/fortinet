...
DATABASE_URL = os.getenv('DATABASE_URL', 'postgresql://itsm:itsm123@localhost:5432/itsm')
...
FORTIMANAGER_SERVICE_URL = os.getenv('FORTIMANAGER_SERVICE_URL', 'http://localhost:8082')
...
class ITSMService:
    ...
    def get_tickets(self, status: str = None, requester: str = None, limit: int = 100) -> list:
        """티켓 목록 조회"""
        if not self.db:
            return []
    ...
    def update_ticket_status(self, ticket_id: int, new_status: str, assignee: str = None) -> bool:
        """티켓 상태 업데이트"""
        if not self.db:
            return False
    ...
    def process_approval(self, ticket_id: int, approver: str, approved: bool, comments: str = None) -> bool:
        """승인 처리"""
        if not self.db:
            return False
            ...
            with self.db.cursor() as cursor:
                ...
                try:
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        deployment_details = {'error': f"FortiManager API error: {response.status_code}"}
                ...
                # 티켓 상태 업데이트
                new_ticket_status = ITSMTicketStatus.DEPLOYED.value if deployment_status == 'success' else ITSMTicketStatus.APPROVED.value
                cursor.execute("""
                    UPDATE itsm_tickets 
                    SET status = %s, updated_at = CURRENT_TIMESTAMP
                    WHERE id = %s
